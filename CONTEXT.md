# CONTEXT — working agreement for this repo (2026-09-21)

Architecture review session. Master plan: `docs/PLAN.md`.
...
- Feature flags: default = `smp,net,ext4,iommu`; CI all-features = `smp,net,ext4,uhci,ash,hypervisor,verification,gpu` (ADR-008).
...
- QEMU gate: `qemu-system-x86_64 -bios OVMF.fd -drive format=raw,file=target\x86_64-vahi\debug\bootimage-vahi_kernel.bin -m 512M -smp N -nographic -serial file:<log> -no-reboot`; expect `TAP version 13` + `# 134/134 passed, 0 failed` at smp 1 and 2.
