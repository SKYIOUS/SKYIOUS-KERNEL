...
fn test_format_mount() -> Result<(), &'static str> {
    ...
    if !root.is_dir() {
        return Err("root should be a dir");
    }
    Ok(())
}
...
fn test_create_file() -> Result<(), &'static str> {
    ...
    if file.is_dir() {
        return Err("file should not be a dir");
    }
    Ok(())
}
...
fn test_mkdir_children() -> Result<(), &'static str> {
    ...
    if !names.iter().any(|n| n == "ino:2") {
        crate::serial_write(&alloc::format!(
            "[skyfs-test] mkdir_children: names={:?}\n",
            names
        ));
    ...
    if !child.is_dir() {
        return Err("subdir should be a dir");
    }
    Ok(())
}
