use crate::task::scheduler::{cpu_sched, GLOBAL};
...
fn test_sleep_queue_add_and_tick() -> Result<(), &'static str> {
    ...
    if GLOBAL.sleep_queue.lock().len() != before + 1 {
        return Err("sleep queue should grow by 1");
    }
    crate::task::scheduler::tick(1_000_000);
    if GLOBAL.sleep_queue.lock().len() != before {
        return Err("thread should be woken at tick=1M");
    }
    Ok(())
}
...
fn test_sleep_queue_not_woken_early() -> Result<(), &'static str> {
    ...
    if GLOBAL.sleep_queue.lock().len() != before + 1 {
        return Err("thread should still be sleeping");
    }
    Ok(())
}
...
fn test_sleep_queue_multiple_wake() -> Result<(), &'static str> {
    ...
    if after != 1 {
        crate::serial_write(&alloc::format!(
            "[sched-test] sleep_multiple_wake: before={} after={}\n",
            before,
            after
        ));
    ...
    if after2 != 0 {
        crate::serial_write(&alloc::format!(
            "[sched-test] sleep_multiple_wake: after2={}\n",
            after2
        ));
...
fn test_futex_queue_wake() -> Result<(), &'static str> {
    ...
    GLOBAL
        .futex_queue
        .lock()
        .push_back(alloc::boxed::Box::new(t1));
    let mut t2 = make_test_thread(5, None);
    t2.futex_wake_addr = Some(0xBEEF);
    GLOBAL
        .futex_queue
        .lock()
        .push_back(alloc::boxed::Box::new(t2));
    if GLOBAL.futex_queue.lock().len() != before + 2 {
        return Err("should have 2 in futex queue");
    }
    let woken = GLOBAL.wake_futex(0xCAFE, 1, &mut *target);
    if woken != 1 {
        return Err("should wake 1 thread");
    }
    if GLOBAL.futex_queue.lock().len() != before + 1 {
        return Err("1 should remain in futex queue");
    }
    Ok(())
}
...
fn test_futex_wake_max() -> Result<(), &'static str> {
    ...
    for _ in 0..5 {
        ...
        GLOBAL
            .futex_queue
            .lock()
            .push_back(alloc::boxed::Box::new(t));
    }
    let woken = GLOBAL.wake_futex(0xF000, 3, &mut *target);
    if woken != 3 {
        return Err("should wake exactly 3");
    }
    if GLOBAL.futex_queue.lock().len() != before + 2 {
        return Err("2 should remain");
    }
    Ok(())
}
...
fn test_block_queue_wake() -> Result<(), &'static str> {
    ...
    if woken != 1 {
        return Err("should wake 1");
    }
    if GLOBAL.block_queue.lock().len() != before {
        return Err("block queue should be empty");
    }
    Ok(())
}
...
fn test_block_queue_wake_wrong_key() -> Result<(), &'static str> {
    ...
    if woken != 0 {
        return Err("should not wake with wrong key");
    }
    if GLOBAL.block_queue.lock().len() != before + 1 {
        return Err("thread should remain");
    }
    Ok(())
}
...
fn test_pick_next_from_pending() -> Result<(), &'static str> {
    ...
    GLOBAL
        .pending_queue
        .lock()
        .push_back(alloc::boxed::Box::new(make_test_thread(5, None)));
    let mut target = cpu_sched(0).ok_or("no cpu 0")?.lock();
    ...
    if thread.is_none() {
        return Err("pick_next should return thread from pending");
    }
    let t = thread.unwrap();
    if t.priority != 5 {
        return Err("picked thread priority mismatch");
    }
    drop(target);
    if GLOBAL.pending_queue.lock().len() != before {
        return Err("pending_queue should be drained");
    }
    Ok(())
}
...
fn test_pick_next_empty() -> Result<(), &'static str> {
    ...
    let thread = target.pick_next();
    if thread.is_some() {
        return Err("pick_next on empty should return None");
    }
    Ok(())
}
...
pub fn register() {
    ...
    crate::selftest::register(
        "sched::sleep_not_woken_early",
        test_sleep_queue_not_woken_early,
    );
    ...
    crate::selftest::register(
        "sched::block_wake_wrong_key",
        test_block_queue_wake_wrong_key,
    );
