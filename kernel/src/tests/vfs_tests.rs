...
fn test_tmpfs_create_file() -> Result<(), &'static str> {
    ...
    if file.is_dir() {
        return Err("file reports is_dir");
    }
...
fn test_tmpfs_create_duplicate() -> Result<(), &'static str> {
    ...
    if root.create("a.txt").is_ok() {
        return Err("duplicate create should fail");
    }
    Ok(())
}
...
fn test_tmpfs_create_on_file() -> Result<(), &'static str> {
    ...
    if file.create("nested").is_ok() {
        return Err("create on file should fail");
    }
    Ok(())
}
...
fn test_tmpfs_mkdir_duplicate() -> Result<(), &'static str> {
    ...
    if root.mkdir("d").is_ok() {
        return Err("duplicate mkdir should fail");
    }
    Ok(())
}
...
fn test_tmpfs_mkdir_on_file() -> Result<(), &'static str> {
    let (_, root) = new_tmpfs();
    let file = root.create("f.txt").map_err(|_| "create failed")?;
    if file.mkdir("sub").is_ok() {
        return Err("mkdir on file should fail");
    }
    Ok(())
}
...
fn test_tmpfs_create_dir_fails() -> Result<(), &'static str> {
    let (_, root) = new_tmpfs();
    if root.create(".").is_ok() {
        return Err("create '.' should fail");
    }
    if root.create("/").is_ok() {
        return Err("create '/' should fail");
    }
...
fn test_tmpfs_mkdir_children() -> Result<(), &'static str> {
    ...
    if !names.iter().any(|n| n == "subdir") {
        return Err("subdir not in children");
    }
    Ok(())
}
...
fn test_tmpfs_rename() -> Result<(), &'static str> {
    ...
    if root.find_child("old.txt").is_some() {
        return Err("old name still exists");
    }
...
fn test_tmpfs_rename_nonexistent() -> Result<(), &'static str> {
    let (_, root) = new_tmpfs();
    if root.rename("nosuch", "new.txt").is_ok() {
        return Err("rename nonexistent should fail");
    }
    Ok(())
}
...
fn test_tmpfs_rename_to_existing() -> Result<(), &'static str> {
    ...
    if root.rename("a.txt", "b.txt").is_ok() {
        return Err("rename to existing should fail");
    }
    Ok(())
}
...
fn test_tmpfs_rename_on_file() -> Result<(), &'static str> {
    let (_, root) = new_tmpfs();
    let file = root.create("f.txt").map_err(|_| "create failed")?;
    if file.rename("x", "y").is_ok() {
        return Err("rename on file should fail");
    }
    Ok(())
}
...
fn test_tmpfs_truncate() -> Result<(), &'static str> {
    ...
    let stat = file.stat().map_err(|_| "stat failed")?;
    if stat.st_size != 5 {
        return Err("size should be 5 after truncate");
    }
    let data = file.read(10).map_err(|_| "read failed")?;
    if data.len() != 5 {
        return Err("read length should be 5");
    }
    if &data[..] != b"12345" {
        return Err("content mismatch after truncate");
    }
    Ok(())
}
...
fn test_tmpfs_truncate_same() -> Result<(), &'static str> {
    ...
    file.truncate(4)
        .map_err(|_| "truncate to same len failed")?;
    let stat = file.stat().map_err(|_| "stat failed")?;
    if stat.st_size != 4 {
        return Err("size should remain 4");
    }
    Ok(())
}
...
fn test_tmpfs_truncate_zero() -> Result<(), &'static str> {
    ...
    let data = file.read(10).map_err(|_| "read failed")?;
    if !data.is_empty() {
        return Err("should be empty after truncate to 0");
    }
    Ok(())
}
...
fn test_tmpfs_unlink() -> Result<(), &'static str> {
    ...
    if after + 1 != before {
        return Err("child count should decrease by 1");
    }
    if root.find_child("delete.me").is_some() {
        return Err("found after unlink");
    }
    Ok(())
}
...
fn test_tmpfs_unlink_nonexistent() -> Result<(), &'static str> {
    let (_, root) = new_tmpfs();
    if root.unlink("nosuch").is_ok() {
        return Err("unlink nonexistent should fail");
    }
    Ok(())
}
...
fn test_tmpfs_unlink_on_file() -> Result<(), &'static str> {
    let (_, root) = new_tmpfs();
    let file = root.create("f.txt").map_err(|_| "create failed")?;
    if file.unlink("child").is_ok() {
        return Err("unlink on file should fail");
    }
    Ok(())
}
...
fn test_tmpfs_nested_create() -> Result<(), &'static str> {
    ...
    let file = sub
        .create("inner.txt")
        .map_err(|_| "create in subdir failed")?;
    if file.is_dir() {
        return Err("inner file reports is_dir");
    }
    let found = sub
        .find_child("inner.txt")
        .ok_or("find_child in subdir failed")?;
    if found.name() != "inner.txt" {
        return Err("name mismatch");
    }
    Ok(())
}
...
fn test_tmpfs_symlink_readlink() -> Result<(), &'static str> {
    let (_, root) = new_tmpfs();
    root.symlink("mylink", "/target/path")
    ...
    if target != "/target/path" {
        return Err("link target mismatch");
    }
    Ok(())
}
...
fn test_tmpfs_write_overwrite() -> Result<(), &'static str> {
    ...
    if data.as_slice() != b"second" {
        return Err("overwrite content mismatch");
    }
    Ok(())
}
...
fn test_tmpfs_write_dir_fails() -> Result<(), &'static str> {
    let (_, root) = new_tmpfs();
    if root.write(b"data").is_ok() {
        return Err("write on root dir should fail");
    }
    let sub = root.mkdir("sub").map_err(|_| "mkdir failed")?;
    if sub.write(b"data").is_ok() {
        return Err("write on subdir should fail");
    }
    Ok(())
}
...
fn test_tmpfs_read_dir_fails() -> Result<(), &'static str> {
    let (_, root) = new_tmpfs();
    if root.read(10).is_ok() {
        return Err("read on root dir should fail");
    }
    let sub = root.mkdir("sub").map_err(|_| "mkdir failed")?;
    if sub.read(10).is_ok() {
        return Err("read on subdir should fail");
    }
    Ok(())
}
...
fn test_tmpfs_multiple_files() -> Result<(), &'static str> {
    ...
    let children = root.children().map_err(|_| "children failed")?;
    if children.len() != 4 {
        return Err("expected 4 children");
    }
    let names: Vec<crate::alloc::string::String> = children.iter().map(|c| c.name()).collect();
