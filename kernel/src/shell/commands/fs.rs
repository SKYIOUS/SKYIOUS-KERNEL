...
pub fn cd(path: &str, out: &mut dyn FnMut(&str)) {
    ...
    let p = format!("{}\0", path);
    if crate::syscalls::syscall_handler(80, p.as_ptr() as u64, 0, 0, 0, 0, core::ptr::null_mut())
        != 0
    {
...
pub fn pwd(out: &mut dyn FnMut(&str)) {
    let mut buf = [0u8; 256];
    let res = crate::syscalls::syscall_handler(
        79,
...
pub fn mkdir(path: &str, out: &mut dyn FnMut(&str)) {
    ...
    let p = format!("{}\0", path);
    if crate::syscalls::syscall_handler(
        83,
        p.as_ptr() as u64,
        0o755,
        0,
...
pub fn rm(path: &str, out: &mut dyn FnMut(&str)) {
    ...
    let p = format!("{}\0", path);
    if crate::syscalls::syscall_handler(87, p.as_ptr() as u64, 0, 0, 0, 0, core::ptr::null_mut())
        != 0
    {
...
pub fn touch(path: &str, out: &mut dyn FnMut(&str)) {
    ...
    let p = format!("{}\0", path);
    let fd = crate::syscalls::syscall_handler(
        2,
        p.as_ptr() as u64,
        0x40,
        0,
...
pub fn cat(filename: &str, out: &mut dyn FnMut(&str)) {
    ...
    match crate::syscalls::fs::sys_open_path(filename) {
        Ok(fd) => {
            ...
            loop {
                let read_len = crate::syscalls::syscall_handler(
                    0,
                    fd,
                    buf.as_mut_ptr() as u64,
                    buf.len() as u64,
...
pub fn stat(filename: &str, out: &mut dyn FnMut(&str)) {
    ...
    if res == 0 {
        let kind = if stat_buf.st_mode & crate::vfs::S_IFDIR != 0 {
            "directory"
        } else {
            "file"
        };
...
pub fn cp(src: &str, dst: &str, out: &mut dyn FnMut(&str)) {
    ...
    let fd_src = crate::syscalls::syscall_handler(
        2,
        src_c.as_ptr() as u64,
        0,
        0,
    ...
    if fd_src >= 0xFFFF_FFFF_FFFF_FF00 {
        ...
        let fd_dst = crate::syscalls::syscall_handler(
            2,
            dst_c.as_ptr() as u64,
            0x41,
            0,
        ...
        if fd_dst >= 0xFFFF_FFFF_FFFF_FF00 {
            ...
            let mut buf = [0u8; 4096];
            loop {
                let n = crate::syscalls::syscall_handler(
                    0,
                    fd_src,
                    buf.as_mut_ptr() as u64,
                    4096,
                ...
                if n == 0 || n >= 0xFFFF_FFFF_FFFF_FF00 {
                    break;
                }
