...
#[cfg(feature = "gpu")]
use crate::compositor::{DirtyRect as CompDirtyRect, GuiScene, GuiWindow};
#[cfg(feature = "gpu")]
    ...
    if !fb_ptr.is_null() {
        unsafe {
            core::ptr::copy_nonoverlapping(
                scene.backbuffer,
                fb_ptr,
                (scene.width * scene.height) as usize,
            );
        }
    }
...
impl Compositor {
    pub fn render(&mut self, mouse_x: usize, mouse_y: usize) {
        ...
        for (i, win) in self.windows.iter().enumerate() {
            ...
            let btn_color = if win.minimized {
                0xFF3A3A3A
            } else if is_active {
                0xFF2D2D2D
            } else {
            ...
            if is_active {
                drawing::draw_line_h(
                    ...
                    taskbar_y + 5,
                    115,
                    crate::gui::accent_color(),
                );
            }
            let display = if win.title.len() > 13 {
                &win.title[..13]
            } else {
                &*win.title
            };
        ...
        for window in &self.windows {
            if !window.minimized {
                window.render(&mut self.backbuffer, mouse_x, mouse_y);
            }
        }
        ...
        for anim in &mut self.animations {
            anim.frame += 1;
            if anim.window_idx >= self.windows.len() {
                continue;
            }
        ...
        if self.alt_tab_active && self.windows.len() > 1 {
            ...
            for (i, win) in self.windows.iter().enumerate() {
                ...
                let bg = if i == self.alt_tab_index {
                    0xFF0078D4
                } else {
                    0xFF3A3A3A
                };
                ...
                let display = if win.title.len() > 14 {
                    &win.title[..14]
                } else {
                    &*win.title
                };
        ...
        #[cfg(not(feature = "gpu"))]
        {
            ...
            if rects.is_empty() {
                ...
                crate::drivers::gpu::virtio_gpu::flip_rect(
                    merge.x as u32,
                    merge.y as u32,
                    merge.w as u32,
                    merge.h as u32,
