...
pub fn sys_mount(
    ...
    if unsafe { user_access::copy_from_user(&mut src_buf[..255], source).is_err() } {
        return errno::Errno::EFAULT as u64;
    }
    if unsafe { user_access::copy_from_user(&mut tgt_buf[..255], target).is_err() } {
        return errno::Errno::EFAULT as u64;
    }
    if unsafe { user_access::copy_from_user(&mut fs_buf[..31], fstype).is_err() } {
        return errno::Errno::EFAULT as u64;
    }
...
pub fn sys_swapon(path_ptr: *const u8, _swap_flags: i32) -> u64 {
    ...
    let read_fn = alloc::sync::Arc::new(move |max_len: usize| -> Result<alloc::vec::Vec<u8>, ()> {
        node_for_read.read(max_len)
    })
        as alloc::sync::Arc<dyn Fn(usize) -> Result<alloc::vec::Vec<u8>, ()> + Send + Sync>;
    let write_fn =
        alloc::sync::Arc::new(move |data: &[u8]| -> Result<(), ()> { node_for_write.write(data) })
            as alloc::sync::Arc<dyn Fn(&[u8]) -> Result<(), ()> + Send + Sync>;
