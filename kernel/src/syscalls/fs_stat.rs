...
use super::errno;
use super::fs_open::{check_chown_permission, split_parent};
use super::*;
        ...
        if let Ok(statfs) = node.statfs() {
            let slice = unsafe {
                core::slice::from_raw_parts(
                    ...
                    core::mem::size_of::<crate::vfs::StatFs>(),
                )
            };
...
pub fn sys_fstat(fd: u64, stat_buf: *mut Stat) -> u64 {
    ...
    match fd_table[fd as usize] {
        ...
        Some(FileDescriptor::TimerFd(_))
        | Some(FileDescriptor::SignalFd(_))
        | Some(FileDescriptor::EventFd(_))
        | Some(FileDescriptor::InotifyFd { .. })
        | Some(FileDescriptor::IoUringFd(_)) => {
            let stat = Stat {
                st_mode: crate::vfs::_S_IFCHR | 0o644,
                ..Stat::default()
            };
            if unsafe {
                user_access::copy_to_user(
                ...
                if let Ok(stat) = entry.object.stat() {
                    if unsafe {
                        user_access::copy_to_user(
                            ...
                            core::slice::from_raw_parts(
                                &stat as *const _ as *const u8,
                                core::mem::size_of::<crate::vfs::Stat>(),
                            ),
                        )
                    }
                    .is_err()
...
pub fn sys_link(old_path_ptr: *const u8, new_path_ptr: *const u8) -> u64 {
    ...
    let parent_node = match vfs.resolve_path(if parent_dir.is_empty() {
        "."
    } else {
        parent_dir
    }) {
    ...
    if parent_node.link(old_node, new_name).is_ok() {
        0
    } else {
        errno::Errno::EPERM as u64
    }
...
pub fn sys_linkat(
    ...
    let old_abs = match resolve_path_at(olddirfd, &old_path, &process) {
        Ok(p) => p,
        Err(e) => return e as u64,
    };
    let new_abs = match resolve_path_at(newdirfd, &new_path, &process) {
        Ok(p) => p,
        Err(e) => return e as u64,
    };
    let vfs = VFS.lock();
    let existing_node = match vfs.resolve_path(&old_abs) {
        Some(n) => n,
        None => return errno::Errno::ENOENT as u64,
    };
    ...
    let (parent_path, name) = if last_slash == 0 {
        ...
        (&new_abs[..last_slash], &new_abs[last_slash + 1..])
    };
...
pub fn sys_rename(old_path_ptr: *const u8, new_path_ptr: *const u8) -> u64 {
    ...
    let (src_parent, src_name) = if src_last == 0 {
        ("/", &old_path[1..])
    } else {
        (&old_path[..src_last], &old_path[src_last + 1..])
    };
    let (dst_parent, dst_name) = if dst_last == 0 {
        ("/", &new_path[1..])
    } else {
        (&new_path[..dst_last], &new_path[dst_last + 1..])
    };
    if src_parent == dst_parent {
        if let Some(parent) = vfs.resolve_path(src_parent) {
            ...
            if parent.rename(src_name, dst_name).is_ok() {
                ...
                super::inotify::inotify_emit(
                    &old_path,
                    super::inotify::IN_MOVED_FROM,
                    cookie,
                    src_name,
                );
                super::inotify::inotify_emit(
                    &new_path,
                    super::inotify::IN_MOVED_TO,
                    cookie,
                    dst_name,
                );
                return 0;
    ...
    let dst_p = match vfs.resolve_path(dst_parent) {
        Some(n) => n,
        None => return errno::Errno::ENOENT as u64,
    };
    if !check_node_permission(&dst_p, 3) {
        return errno::Errno::EACCES as u64;
    }
    if dst_p.create(dst_name).is_err() {
        return errno::Errno::EIO as u64;
    }
    if let Some(new_node) = dst_p.find_child(dst_name) {
        let _ = new_node.write(&data);
    }
    let src_p = match vfs.resolve_path(src_parent) {
        Some(n) => n,
        None => return errno::Errno::ENOENT as u64,
    };
...
pub fn sys_renameat(
    ...
    let (src_parent, src_name) = if src_last == 0 {
        ...
        (&abs_old[..src_last], &abs_old[src_last + 1..])
    };
    let (dst_parent, dst_name) = if dst_last == 0 {
        ("/", &abs_new[1..])
    } else {
        (&abs_new[..dst_last], &abs_new[dst_last + 1..])
    };
    if src_parent == dst_parent {
        if let Some(parent) = vfs.resolve_path(src_parent) {
            if !check_node_permission(&parent, 3) {
...
pub fn sys_mkdir(path_ptr: *const u8, mode: u32) -> u64 {
    ...
    if let Some(parent_node) = vfs.resolve_path(parent_path) {
        ...
        if let Ok(new_node) = parent_node.mkdir(name) {
            ...
            super::inotify::inotify_emit(
                &path_str,
                super::inotify::IN_CREATE | super::inotify::IN_ISDIR,
                0,
                name,
            );
            return 0;
...
pub fn sys_utimensat(
    ...
    if node.utimens((now.0, now.1), (now.0, now.1)).is_ok() {
        0
    } else {
        errno::Errno::EPERM as u64
    }
