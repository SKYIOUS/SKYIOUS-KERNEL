...
/// prctl() syscall
pub fn sys_prctl(option: u64, arg2: u64, arg3: u64, arg4: u64, _arg5: u64) -> u64 {
    // Clone the process out of the guard: PR_SET_SECCOMP delegates to
    // sys_seccomp, which locks CURRENT_PROCESS itself — a guard held across
    // the delegation would self-deadlock (non-reentrant mutex family).
    let proc = {
    ...
    match option {
        PR_SET_NO_NEW_PRIVS => {
            ...
            proc.no_new_privs
                .store(arg2 != 0, core::sync::atomic::Ordering::SeqCst);
        ...
        PR_SET_CHILD_SUBREAPER => {
            ...
            proc.child_subreaper
                .store(is_subreaper, core::sync::atomic::Ordering::SeqCst);
        ...
        PR_GET_CHILD_SUBREAPER => {
            let val = proc
                .child_subreaper
                .load(core::sync::atomic::Ordering::SeqCst) as u64;
            val
        }

        PR_SET_TIMERSLACK => {
            proc.timerslack
                .store(arg2 as u64, core::sync::atomic::Ordering::SeqCst);
            0
        }

        PR_GET_TIMERSLACK => proc.timerslack.load(core::sync::atomic::Ordering::SeqCst),
        ...
        PR_GET_OOM_SCORE_ADJ => crate::task::oom::get_oom_score_adj(proc.id) as u64,
