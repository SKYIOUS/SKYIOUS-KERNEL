...
pub fn sys_futex(
    ...
    match op {
        ...
        FUTEX_REQUEUE => {
            ...
            if woken > 0 {
                scheduler::wake_futex(uaddr2 as u64, _val2);
            }
            woken as u64
        }
...
fn signal_pending() -> bool {
    ...
    lock.as_ref()
        .map(|p| p.signals.lock().has_pending())
        .unwrap_or(false)
}
...
fn futex_lock_pi(uaddr: *mut u32) -> u64 {
    ...
    if prev == 0 {
        ...
        if old == 0 {
            let prio = scheduler::this_cpu_sched()
                .lock()
                .current_thread
                .as_ref()
                .map(|t| t.priority)
    ...
    if current & FUTEX_WAITERS == 0 {
        unsafe {
            core::ptr::write_volatile(uaddr, current | FUTEX_WAITERS);
        }
    }
    ...
    if owner_pid != 0 {
        let our_prio = scheduler::this_cpu_sched()
            .lock()
            .current_thread
            .as_ref()
            .map(|t| t.priority)
