...
fn cur_pid() -> u64 {
    CURRENT_PROCESS.lock().as_ref().map_or(0, |p| p.id)
}
...
pub fn mq_send(mqd: i32, msg_ptr: *const u8, msg_len: usize, prio: u32) -> u64 {
    ...
    let mut q = QUEUES.lock();
    let queue = match q.get_mut(&qn) {
        Some(q) => q,
        None => return -(errno::Errno::ENOENT as i64) as u64,
    };
    if queue.deleted {
        return -(errno::Errno::EIDRM as i64) as u64;
    }
    if msg_len > queue.max_msg_size {
        return -(errno::Errno::EMSGSIZE as i64) as u64;
    }
    if queue.msgs.len() >= queue.max_msgs {
        if (qf & O_NONBLOCK) != 0 {
            return -(errno::Errno::EAGAIN as i64) as u64;
        }
        drop(q);
...
pub fn mq_close(mqd: i32) -> u64 {
    ...
    if !fds.values().any(|f| f.qname == qname) {
        if q.get(&qname).map_or(false, |q| q.deleted) {
            q.remove(&qname);
        }
    }
    0
...
pub fn mq_unlink(name_ptr: *const u8) -> u64 {
    let name = match unsafe { user_access::read_user_string(name_ptr, 256) } {
        Ok(s) => s,
        Err(_) => return -(errno::Errno::EINVAL as i64) as u64,
    };
...
pub fn mq_close_all(pid: u64) {
    ...
    for name in names {
        if !fds.values().any(|f| f.qname == name) {
            if q.get(&name).map_or(false, |q| q.deleted) {
                q.remove(&name);
            }
        }
    }
