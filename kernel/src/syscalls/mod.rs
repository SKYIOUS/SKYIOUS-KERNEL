...
pub static PER_CPU_AREAS: crate::sync::IrqSafeMutex<alloc::vec::Vec<PerCpuPtr>> =
...
pub struct PerCpuData {
    ...
    /// Callee-saved GPRs (rbx, rbp, r12-r15) at page-fault dispatch entry,
    /// written by the `vahi_pf_dispatch` trampoline for ring-0 faults. The
    /// x86-interrupt handler clobbers these while running; `abort_user_copy`
    /// restores them before iretq'ing to the fixup so the interrupted copy
    /// routine's caller sees the registers the ABI promises were preserved.
    pub pf_callee_saved: [u64; 6], // offset 0x50
}
...
/// Offset of `PerCpuData::pf_callee_saved`, written by the page-fault dispatch
/// trampoline (asm) and read by `abort_user_copy`.
pub const PF_CALLEE_SAVED_OFFSET: usize = 0x50;
const _: () = assert!(core::mem::offset_of!(PerCpuData, pf_callee_saved) == PF_CALLEE_SAVED_OFFSET);
...
use crate::task::process::{MINSIGSTKSZ, SS_DISABLE, SS_ONSTACK};
...
use crate::task::process::{EventFdData, EFD_MAX, EFD_NONBLOCK, EFD_SEMAPHORE};
use crate::task::process::{SignalFdData, SignalFdInfo, SIGNAL_FDS};
