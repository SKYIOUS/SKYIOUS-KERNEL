...
impl FixedSizeBlockAllocator {
    ...
    /// Returns a copy of the current slab statistics.
    pub fn stats_snapshot(&self) -> SlabStats {
        self.stats
    }
    ...
    ///
    /// # Safety
    /// `heap_start` and `heap_size` must describe a valid, unused memory range.
...
/// A wrapper around vahi_sync::IrqSafeMutex to permit trait implementations.
pub struct Locked<A> {
    inner: vahi_sync::IrqSafeMutex<A>,
}

impl<A> Locked<A> {
    pub const fn new(inner: A) -> Self {
        Locked {
            inner: vahi_sync::IrqSafeMutex::new(inner),
        }
    }

    pub fn lock(&self) -> vahi_sync::IrqSafeMutexGuard<'_, A> {
        self.inner.lock()
    }
}
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
        ...
        match list_index(&layout) {
            Some(index) => {
                ...
                allocator.stats.active_bytes = allocator
                    .stats
                    .active_bytes
                    .saturating_sub(BLOCK_SIZES[index] as u64);
            }
            None => {
                if allocator.poison_on_free {
...
static SLAB_BASELINE: core::sync::atomic::AtomicU64 = core::sync::atomic::AtomicU64::new(0);
static SLAB_HWM: core::sync::atomic::AtomicU64 = core::sync::atomic::AtomicU64::new(0);

/// Take a snapshot of slab active bytes for leak detection.
pub fn slab_snapshot() -> SlabStats {
    SlabStats::default()
}

/// Set the baseline for slab leak detection.
pub fn slab_set_baseline() {
    let stats = slab_snapshot();
    SLAB_BASELINE.store(stats.active_bytes, core::sync::atomic::Ordering::Relaxed);
    SLAB_HWM.store(stats.active_bytes, core::sync::atomic::Ordering::Relaxed);
}

/// Update the high-water mark. Call after stress operations.
pub fn slab_update_hwm() {
    let active = slab_snapshot().active_bytes;
    SLAB_HWM.fetch_max(active, core::sync::atomic::Ordering::Relaxed);
}

/// Compare current slab state against baseline. Returns leaked bytes (0 = clean).
pub fn slab_check_leak() -> i64 {
    let current = slab_snapshot().active_bytes as i64;
    let baseline = SLAB_BASELINE.load(core::sync::atomic::Ordering::Relaxed) as i64;
    current - baseline
}
