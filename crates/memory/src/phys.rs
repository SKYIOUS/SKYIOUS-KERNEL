...
pub fn total_free_frames() -> usize {
    crate::buddy::BUDDY_ALLOCATOR.lock().count_free_pages()
}
...
pub fn audit_snapshot() -> MemoryAudit {
    ...
    MemoryAudit {
        ...
        peak_usage_frames: if baseline != usize::MAX {
            baseline.saturating_sub(hwm)
        } else {
            0
        },
        current_leak: if baseline != usize::MAX {
            baseline.saturating_sub(free)
        } else {
            0
        },
        min_free: lwm,
