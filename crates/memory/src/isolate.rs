...
impl VmFlags {
    pub fn to_page_table_flags(self) -> PageTableFlags {
        ...
        if !self.contains(VmFlags::EXEC) {
            f.insert(PageTableFlags::NO_EXECUTE);
        }
        f
    }
}
...
impl Isolate {
    ...
    pub fn mmap_at(
        ...
        if self.overlaps(virt, page_bytes) {
            return None;
        }
        ...
        for i in 0..page_count as u64 {
            ...
            unsafe {
                mapper
                    .map_to(page, frame, pt_flags, frame_allocator)
                    .ok()?
                    .ignore();
            }
        }
    ...
    pub fn mprotect(&mut self, virt: u64, new_flags: VmFlags) -> bool {
        let Some(region) = self.regions.get_mut(&virt) else {
            return false;
        };
        region.flags = new_flags;
        let pt_flags = new_flags.to_page_table_flags();
        let page_count = region.size.div_ceil(4096);

        if let Some(mut mapper) = unsafe { self.address_space.mapper() } {
            for i in 0..page_count as u64 {
                let page = Page::<Size4KiB>::containing_address(VirtAddr::new(virt + i * 4096));
                unsafe {
                    mapper.update_flags(page, pt_flags).ok();
                }
            }
        }
        true
    ...
    pub fn brk(
        ...
        while self.heap_end < new_end {
            ...
            if let Some(mut mapper) = unsafe { self.address_space.mapper() } {
                unsafe {
                    let pt_flags = flags.to_page_table_flags();
                    if let Ok(r) = mapper.map_to(page, frame, pt_flags, frame_allocator) {
                        r.ignore()
                    }
                }
            }
