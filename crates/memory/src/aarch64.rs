...
fn clone_pt_entry_tree(entry: u64, _index: usize, phys_offset: u64, level: usize) -> u64 {
    if level >= 4 {
...
pub fn identity_map_range(start_pa: u64, end_pa: u64, table_l0_pa: u64) -> Option<()> {
    let phys_offset = *crate::PHYSICAL_MEMORY_OFFSET.get()?;
...
fn walk_or_create(table_pa: u64, index: usize, phys_offset: u64) -> Option<u64> {
    unsafe {
...
fn buddy_alloc_frame_raw() -> Option<u64> {
    crate::buddy::allocate_raw_frame_addr().map(|pa| {
        let phys_offset = crate::PHYSICAL_MEMORY_OFFSET.get().copied().unwrap_or(0);
...
mod tests {
    ...
    fn test_make_table_desc() {
        ...
        assert_eq!(
            ...
            child_pa & 0x0000_FFFF_FFFF_F000
        );
    }
    ...
    fn test_make_pte_physical_address_alignment() {
        ...
        assert_eq!(
            pte & 0xFFF,
            0x3 | ATTR_ACCESS | ATTR_NON_GLOBAL,
            "lower bits should only be flags"
        );
        ...
        assert_eq!(
            ...
            0x1000,
            "should round down to page boundary"
        );
    }
