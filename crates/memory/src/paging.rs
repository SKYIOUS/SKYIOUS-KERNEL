...
impl AddressSpace {
    ...
    pub fn new(frame_allocator: &mut impl FrameAllocator<Size4KiB>) -> Option<Self> {
        ...
        let phys_offset = *crate::PHYSICAL_MEMORY_OFFSET.get()?;
        ...
        let new_pml4 = unsafe { &mut *(new_pml4_virt.as_mut_ptr::<PageTable>()) };
    ...
    ///
    /// # Safety
    /// The PML4 frame must be valid and mapped. Must not be called from IRQ context.
    ...
    ///
    /// # Safety
    /// The returned mapper must only be used while this address space is active.
    ...
    pub fn clone_cow(&self, frame_allocator: &mut impl FrameAllocator<Size4KiB>) -> Option<Self> {
        ...
        let old_pml4 = unsafe { &mut *(old_pml4_virt.as_mut_ptr::<PageTable>()) };
    ...
    fn clone_recursive(
        ...
        if level == 1 {
            ...
            crate::frame_info::increment(entry.addr());
        ...
        let next_src_table = unsafe { &mut *(next_src_virt.as_mut_ptr::<PageTable>()) };

        let next_dst_virt = virt_offset + new_frame.start_address().as_u64();
        let next_dst_table = unsafe { &mut *(next_dst_virt.as_mut_ptr::<PageTable>()) };
        next_dst_table.zero();
            ...
            if !pml4[p4_idx].is_unused() {
                ...
                if flags.contains(PageTableFlags::HUGE_PAGE) {
                    ...
                    if let Ok(frame) = pml4[p4_idx].frame() {
                        crate::frame_info::decrement(frame.start_address());
                    }
                    continue;
                }
                if let Ok(p3_frame) = pml4[p4_idx].frame() {
                    let p3_virt = virt_offset + p3_frame.start_address().as_u64();
                    let p3_table = unsafe { &*(p3_virt.as_ptr::<PageTable>()) };
        ...
        for i in 0..512 {
            ...
            if level == 1 || flags.contains(PageTableFlags::HUGE_PAGE) {
                ...
                if let Ok(child_frame) = table[i].frame() {
                    ...
                    let child_table = unsafe { &*(child_virt.as_ptr::<PageTable>()) };
    ...
    ///
    /// # Safety
    /// `page` must be a valid page within this address space. The page fault handler must have already
    ...
    pub unsafe fn handle_cow(&self, page: Page<Size4KiB>) -> Option<bool> {
        ...
        let p3_table = &mut *((virt_offset + p3_table_frame.start_address().as_u64())
            .as_mut_ptr::<PageTable>());

        let p2_table_frame = p3_table[p3_idx].frame().ok()?;
        let p2_table = &mut *((virt_offset + p2_table_frame.start_address().as_u64())
            .as_mut_ptr::<PageTable>());

        let p1_table_frame = p2_table[p2_idx].frame().ok()?;
        let p1_table = &mut *((virt_offset + p1_table_frame.start_address().as_u64())
            .as_mut_ptr::<PageTable>());
        ...
        let ref_count = crate::frame_info::count(old_frame.start_address());

        // Promote flags: make writable and clear the COW bit (bit 9).
        let writable_flags = {
            let mut f = flags;
            f.insert(PageTableFlags::WRITABLE);
            PageTableFlags::from_bits_truncate(f.bits() & !(1 << 9))
        };
        ...
        if ref_count > 1 {
            ...
            crate::frame_info::decrement(old_frame.start_address());
        } else {
            // Only one reference — promote in place
