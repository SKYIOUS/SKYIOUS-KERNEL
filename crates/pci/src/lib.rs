    ...
    for bus in 0..=255 {
        for device in 0..32 {
            for function in 0..8 {
                ...
                let bar0 = read_config_u32(bus, device, function, 0x10);
                let bar1 = read_config_u32(bus, device, function, 0x14);
                let bar2 = read_config_u32(bus, device, function, 0x18);
                let bar3 = read_config_u32(bus, device, function, 0x1C);
                let bar4 = read_config_u32(bus, device, function, 0x20);
                let bar5 = read_config_u32(bus, device, function, 0x24);
...
pub fn pci_enable_msi(dev: &PciDevice, target_cpu: u8) -> Result<u8, ()> {
    let cap = find_capability(dev, 0x05).ok_or(())?; // MSI Cap ID = 0x05

    let vector = vahi_apic::msi::alloc().ok_or(())?;
