use crate::{FileSystem, Stat, VfsNode, S_IFDIR, S_IFLNK, S_IFREG};
...
fn parse_tar(data: &[u8]) -> Arc<TarNode> {
    ...
    while offset + 512 <= data.len() {
        ...
        let file_data = if !is_dir && size > 0 {
            &data[offset + 512..end]
        } else {
            &[]
        };
    ...
    crate::vfs_serial_write("[TARFS] parsed ");
    let mut b = [0u8; 12];
    let mut i = 12u8;
    let mut n = count;
    loop {
        i -= 1;
        b[i as usize] = b'0' + (n % 10) as u8;
        n /= 10;
        if n == 0 {
            break;
        }
    }
    crate::vfs_serial_write(core::str::from_utf8(&b[i as usize..]).unwrap_or("?"));
    crate::vfs_serial_write(" entries\n");
    root
}
...
fn add_to_tree(
    ...
    for (i, comp) in components.iter().enumerate() {
        ...
        if let Some(existing) = next {
            ...
            let node_link = if is_last && is_symlink {
                link_target.clone()
            } else {
                None
            };
...
impl Tarfs {
    pub fn new(device: Arc<Mutex<dyn BlockDevice>>) -> Result<Self, ()> {
        ...
        loop {
            let mut buf = [0u8; 512];
            if device.lock().read_sector(block, &mut buf).is_err() {
                break;
            }
            if block == 0 && buf[0] != b'.' && buf[0] != b'/' {
                return Err(());
            }
