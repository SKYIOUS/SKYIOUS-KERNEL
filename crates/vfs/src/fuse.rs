...
impl FuseFs {
    ...
    fn request(&self, opcode: u32, node_id: u64, data: &[u8]) -> Result<Vec<u8>, ()> {
        ...
        // SAFETY: FuseRequestHeader is repr(C) — reinterpreting as bytes
        // for the FUSE protocol is valid.
        let hdr_bytes = unsafe {
            core::slice::from_raw_parts(
                &header as *const _ as *const u8,
                core::mem::size_of::<FuseRequestHeader>(),
            )
        };
        ...
        // ponytail: FUSE needs eventfd to signal userspace and block on replies.
        // Deferred until eventfd plumbing is wired into the VFS layer.
        Err(())
    }
}
...
impl VfsNode for FuseNode {
    ...
    fn read(&self, max_len: usize) -> Result<Vec<u8>, ()> {
        ...
        let header = FuseRequestHeader {
            len: (core::mem::size_of::<FuseRequestHeader>() + core::mem::size_of::<FuseReadIn>())
                as u32,
            opcode: FUSE_READ,
            unique: 0,
            node_id: self.ino,
        ...
        // SAFETY: FUSE protocol structs are repr(C) — byte serialization is valid.
        ...
        data.extend_from_slice(unsafe {
            core::slice::from_raw_parts(
                &read_in as *const _ as *const u8,
    ...
    fn write(&self, data: &[u8]) -> Result<(), ()> {
        let _fs = self.fs.lock();
        let header = FuseRequestHeader {
            len: (core::mem::size_of::<FuseRequestHeader>()
        ...
        req.extend_from_slice(unsafe {
            core::slice::from_raw_parts(
                &write_in as *const _ as *const u8,
                core::mem::size_of::<FuseWriteIn>(),
            )
        });
    ...
    fn children(&self) -> Result<Vec<Arc<dyn VfsNode>>, ()> {
        ...
        // SAFETY: FuseRequestHeader is repr(C) — byte serialization is valid.
        let hdr_bytes = unsafe {
            core::slice::from_raw_parts(
                &header as *const _ as *const u8,
                core::mem::size_of::<FuseRequestHeader>(),
        ...
        while offset + core::mem::size_of::<FuseDirent>() <= reply.len() {
            // SAFETY: offset + sizeof(FuseDirent) <= reply.len(). FuseDirent is
            // repr(C) and read_unaligned is not needed as read handles alignment.
            let dent: FuseDirent =
                unsafe { core::ptr::read(reply[offset..].as_ptr() as *const FuseDirent) };
...
pub fn init() {
    crate::vfs_serial_write("[FUSE] FUSE bridge initialized\n");
}
