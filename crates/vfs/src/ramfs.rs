...
impl VfsNode for TmpNode {
    ...
    fn statfs(&self) -> Result<crate::StatFs, ()> {
        ...
        Ok(crate::StatFs {
            ...
            f_bfree: total_blocks - 1,
            f_bavail: total_blocks - 1,
            f_files: 4096,
            f_ffree: 4096 - 4,
        })
    }
    ...
    fn link(&self, existing: alloc::sync::Arc<dyn VfsNode>, name: &str) -> Result<(), ()> {
        ...
        if stat.st_mode & crate::S_IFDIR != 0 {
            return Err(());
        }
    ...
    fn utimens(&self, atime: (i64, i64), mtime: (i64, i64)) -> Result<(), ()> {
        ...
        // UTIME_OMIT = -1: leave unchanged; UTIME_NOW = -2: set to current time (handled in syscall layer)
        if atime.1 != -1 && atime.1 != -2 {
            *self.atime_nsec.lock() = atime.1;
        }
        if mtime.1 != -1 && mtime.1 != -2 {
