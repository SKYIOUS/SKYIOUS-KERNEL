...
use crate::{FileSystem, Stat, StatFs, VfsNode};
...
pub struct SkyFS {
    ...
    pub sb: Superblock,
    pub journal: vahi_sync::IrqSafeMutex<journal::Journal>,
}
...
impl SkyFS {
    pub fn format(device: &Arc<Mutex<dyn BlockDevice>>) -> Result<(), ()> {
        ...
        // SAFETY: Superblock is repr(C, packed) with all-zeros padding.
        // Reinterpreting its bytes as a u8 slice is valid for any bit pattern.
        let sb_src = unsafe {
        ...
        // SAFETY: SkyfsInode is repr(C, packed). INODE_SIZE = sizeof(SkyfsInode).
        // Reinterpreting as bytes for disk serialization is valid.
        let src = unsafe {
        ...
        // SAFETY: root_dst is INODE_SIZE bytes within the block buffer,
        // read from disk into a valid SkyfsInode location.
        ...
        // SAFETY: SkyfsInode is repr(C, packed) — any bit pattern valid for serialization.
        let root_src = unsafe {
    ...
    pub fn mount(device: Arc<Mutex<dyn BlockDevice>>) -> Result<Arc<Mutex<SkyFS>>, ()> {
        ...
        for i in 0..SECTORS_PER_BLOCK {
            dev.read_sector(
                i,
                &mut sb_buf[i as usize * SECTOR_SIZE..(i as usize + 1) * SECTOR_SIZE],
            )
            .map_err(|_| ())?;
        }
        drop(dev);
        // SAFETY: sb_buf is a full block read from sector 0. Superblock is
        // repr(C, packed) with total size <= BLOCK_SIZE (4096 bytes).
        // Magic check below rejects garbage data.
        ...
        let fs = Arc::new(Mutex::new(SkyFS {
            device,
            journal: vahi_sync::IrqSafeMutex::new(journal::Journal::new(j_start, j_blocks)),
            sb: Superblock {
                magic: sb.magic,
                version: sb.version,
    ...
    pub fn block_to_sector(block: u64) -> u64 {
    ...
    pub fn read_block(dev: &mut dyn BlockDevice, block: u64, buf: &mut [u8]) -> Result<(), ()> {
        ...
        for i in 0..SECTORS_PER_BLOCK {
            ...
            dev.read_sector(
                sector,
                &mut buf[i as usize * SECTOR_SIZE..(i as usize + 1) * SECTOR_SIZE],
            )
            .map_err(|_| ())?;
        }
        Ok(())
    ...
    pub fn set_state(dev: &mut dyn BlockDevice, state: u8) -> Result<(), ()> {
        ...
        for i in 0..SECTORS_PER_BLOCK {
            dev.write_sector(
                i,
                &sb_buf[i as usize * SECTOR_SIZE..(i as usize + 1) * SECTOR_SIZE],
            )
            .map_err(|_| ())?;
        }
        Ok(())
...
fn read_inode_inner(fs: &Arc<Mutex<SkyFS>>, ino: u64) -> Result<SkyfsInode, ()> {
    ...
    // SAFETY: buf[offset..] has at least INODE_SIZE bytes remaining.
    // SkyfsInode is repr(C, packed) — reinterpreting as a reference is valid.
...
fn write_inode_inner(fs: &Arc<Mutex<SkyFS>>, ino: u64, inode: &SkyfsInode) -> Result<(), ()> {
    ...
    // SAFETY: inode is a valid SkyfsInode. INODE_SIZE = sizeof(SkyfsInode).
    // repr(C, packed) — reinterpreting as bytes for disk serialization.
    let src =
        unsafe { core::slice::from_raw_parts(inode as *const SkyfsInode as *const u8, INODE_SIZE) };
    dst.copy_from_slice(src);
...
fn read_file_data(fs: &Arc<Mutex<SkyFS>>, inode: &SkyfsInode) -> Result<Vec<u8>, ()> {
    ...
    if inode.extent_count == BTREE_MODE {
        ...
        // SAFETY: inode.data contains packed Extent structs. extent_count is
        // validated by write to be <= MAX_INLINE_DATA / sizeof(Extent).
        let extents = unsafe {
            core::slice::from_raw_parts(
                inode.data.as_ptr() as *const Extent,
                inode.extent_count as usize,
            )
        };
        let mut data = Vec::with_capacity(size);
        for ext in extents {
...
fn store_data_blocks(
    ...
    let blocks_needed = data_len.div_ceil(BLOCK_SIZE);
    let mut extents: Vec<Extent> = alloc::vec![];
    let mut offset = 0;
    let dev_arc = fs.lock().device.clone();
    let mut dev = dev_arc.lock();
    for _ in 0..blocks_needed {
        let block = block_alloc::allocate_block_inner(fs, &mut *dev)?;
        let mut block_buf = [0u8; BLOCK_SIZE];
    ...
    // SAFETY: Extent is repr(C, packed) with Copy fields — any bit pattern valid.
    ...
    if extent_count * core::mem::size_of::<Extent>() <= MAX_INLINE_DATA
...
impl SkyfsNode {
    ...
    fn read_blocks_direct(
        ...
        let blocks_to_read = buf.len().div_ceil(BLOCK_SIZE);
        ...
        if inode.extent_count == BTREE_MODE {
            ...
            while blocks_read < blocks_to_read {
                if let Some(extent) = btree::lookup_extent(&self.fs, root_block, logical_idx) {
                    for b in 0..extent.block_count {
                        ...
                        let dst =
                            &mut buf[blocks_read * BLOCK_SIZE..(blocks_read + 1) * BLOCK_SIZE];
            ...
            // SAFETY: inode.data contains packed Extent structs written by
            // store_data_blocks. extent_count is validated by the writer.
            let extents = unsafe {
                core::slice::from_raw_parts(
                    inode.data.as_ptr() as *const Extent,
                    inode.extent_count as usize,
...
impl VfsNode for SkyfsNode {
    ...
    fn read(&self, max_len: usize) -> Result<Vec<u8>, ()> {
        use crate::page_cache::{GLOBAL_PAGE_CACHE, PAGE_SIZE};
        ...
        while current_offset < len {
            ...
            let page_arc = if let Some(p) = GLOBAL_PAGE_CACHE.get_page(self.ino, page_idx) {
                ...
                self.read_blocks_direct(
                    &inode,
                    page_idx * (PAGE_SIZE as u64 / BLOCK_SIZE as u64),
                    &mut buf,
                )?;
    ...
    fn unlink(&self, name: &str) -> Result<(), ()> {
        ...
        dir::parse_entries(&data, |entry, entry_name| {
            if entry_name == name {
                ...
                write_dirent_append(&mut new_data, &entry, entry_name.as_bytes());
            }
        });
        if !found {
            return Err(());
...
fn write_dirent_append(buf: &mut Vec<u8>, entry: &DirEntry, name: &[u8]) {
    // SAFETY: DirEntry is repr(C, packed) with Copy fields — any bit pattern
    // is valid. Reinterpreting as bytes for serialization is safe.
...
fn write_dirent(buf: &mut [u8], off: &mut usize, entry: &DirEntry, name: &[u8]) -> Result<(), ()> {
    ...
    // SAFETY: DirEntry is repr(C, packed) — serialization as bytes is valid.
    let entry_bytes = unsafe {
        core::slice::from_raw_parts(
            &entry as *const DirEntry as *const u8,
            core::mem::size_of::<DirEntry>(),
