...
#[cfg(feature = "verification")]
lazy_static! {
    static ref JOURNAL_VERIFIER: Mutex<JournalStateMachine> =
        Mutex::new(JournalStateMachine::new());
}
...
impl Journal {
    ...
    pub fn init_device(
        ...
        // SAFETY: JournalHeader is repr(C, packed) — serialization as bytes is valid.
        let src = unsafe {
    ...
    pub fn journal_data(
        ...
        // SAFETY: hdr_buf is BLOCK_SIZE bytes read from the journal header block.
        // JournalHeader is repr(C, packed) and fits within one block.
        ...
        if entry_offset + entry_size <= BLOCK_SIZE {
            // SAFETY: entry_offset + entry_size <= BLOCK_SIZE, and the dst is
            // within hdr_buf. JournalTarget is repr(C, packed) — byte copy is valid.
            unsafe {
    ...
    pub fn commit_transaction(
        ...
        // SAFETY: buf is a BLOCK_SIZE block read from the journal header.
    ...
    pub fn recover_from_dev(dev: &mut dyn BlockDevice, journal: &mut Journal) -> Result<u32, ()> {
        ...
        for i in 0..journal.num_blocks {
            ...
            // SAFETY: buf is BLOCK_SIZE bytes from journal block. JournalHeader
            // is repr(C, packed) and fits within one block.
            ...
            if hdr.checksum != 0 && hdr.checksum != expected_cs {
                vfs_debug!("JOURNAL: checksum mismatch at block {}, skipping", block);
                continue;
            }
            ...
            for t in 0..hdr.num_targets as usize {
                ...
                // SAFETY: entry_offset + entry_size <= BLOCK_SIZE. JournalTarget
                // is repr(C, packed) and reads from the journal block buffer.
                let target: &JournalTarget =
                    unsafe { &*(buf[entry_offset..].as_ptr() as *const JournalTarget) };
                ...
                vfs_debug!(
                    "JOURNAL: replayed block {} → target block {}",
                    journal_data_block,
                    tblock
                );
                replayed_count += 1;
            ...
            // SAFETY: write_buf is BLOCK_SIZE bytes, reinterpret as JournalHeader.
            let hdr_mut: &mut JournalHeader =
                unsafe { &mut *(write_buf.as_mut_ptr() as *mut JournalHeader) };
...
pub fn journal_write_entry(
    ...
    let mut payload = alloc::vec![0u8; BLOCK_SIZE];
    ...
    let hdr_bytes = unsafe {
        core::slice::from_raw_parts(
            &ehdr as *const JournalEntryHeader as *const u8,
