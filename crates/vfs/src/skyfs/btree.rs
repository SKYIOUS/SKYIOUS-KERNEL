extern crate alloc;
use super::{Extent, SkyFS, BLOCK_SIZE};
use alloc::sync::Arc;
...
pub fn lookup_extent(fs: &Arc<Mutex<SkyFS>>, root_block: u64, block_offset: u64) -> Option<Extent> {
    ...
    loop {
        ...
        if nk > KEY_MAX {
            return None;
        } // corrupt node, never index out of bounds
        if node.is_leaf != 0 {
            for i in 0..nk {
...
fn insert_non_full(
    ...
    key: u64,
    value: Extent,
) -> Result<Option<(u64, Extent, u64)>, ()> {
    let mut node = read_node_from_dev(dev, node_block)?;
    let nk = node.num_keys as usize;
    if nk > KEY_MAX {
        return Err(());
    } // corrupt disk node — refuse
...
fn alloc_node_block(fs: &Arc<Mutex<SkyFS>>, dev: &mut dyn BlockDevice) -> Result<u64, ()> {
    super::block_alloc::allocate_block_inner(fs, dev)
}
...
fn read_node(fs: &Arc<Mutex<SkyFS>>, block: u64) -> Option<BTreeNode> {
    ...
    // SAFETY: buf is a BLOCK_SIZE buffer read from disk. BTreeNode is
    // repr(C, packed) and read_unaligned handles any alignment.
...
fn write_node(dev: &mut dyn BlockDevice, block: u64, node: &BTreeNode) -> Result<(), ()> {
    let mut buf = [0u8; BLOCK_SIZE];
    // SAFETY: BTreeNode is repr(C, packed) — reinterpreting as bytes
    // for disk serialization is valid for any bit pattern.
    let src = unsafe {
