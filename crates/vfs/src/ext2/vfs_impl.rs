...
impl VfsNode for Ext2Node {
    ...
    fn children(&self) -> Result<Vec<Arc<dyn VfsNode>>, ()> {
        ...
        for &b in &fs.read_all_block_indices(&self.inode)? {
            ...
            while off < fs.block_size {
                // SAFETY: data is block_size bytes from disk. off < block_size.
                let e = unsafe { &*(data.as_ptr().add(off) as *const DirectoryEntry) };
                if e.inode == 0 {
                    break;
                }
                // SAFETY: name bytes at off+8, e.name_len bytes, within block.
    ...
    fn mkdir(&self, name: &str) -> Result<Arc<dyn VfsNode>, ()> {
        ...
        // SAFETY: block_data is block_size bytes (>= 24).
        // '.' entry at offset 0, 12 bytes (8 header + 1 name + 3 padding).
        {
        ...
        // SAFETY: '..' entry at offset 12. rec_len covers remainder of block.
        {
    ...
    fn readlink(&self) -> Result<String, ()> {
        ...
        if size <= 60 {
            // SAFETY: For symlinks with size <= 60, the target is stored inline
            // in i_block (15 * 4 = 60 bytes). addr_of! gets a pointer to the
            // field without creating a reference. from_raw_parts reads size bytes.
    ...
    fn symlink(&self, name: &str, target: &str) -> Result<(), ()> {
        ...
        if tgt.len() <= 60 {
            // SAFETY: i_block is 15 * 4 = 60 bytes. tgt.len() <= 60.
            // addr_of_mut! gets a mutable pointer without creating a reference.
    ...
    fn truncate(&self, len: i64) -> Result<(), ()> {
        ...
        if new_len > 0 && !new_len.is_multiple_of(bs) {
