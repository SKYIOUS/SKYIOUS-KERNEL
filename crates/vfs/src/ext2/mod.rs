...
impl Ext2FileSystem {
    pub fn new(device: Arc<Mutex<dyn BlockDevice>>) -> Result<Arc<Mutex<Self>>, ()> {
        ...
        // SAFETY: buf is a 1024-byte sector read from disk at sector 2
        // (the superblock location). Superblock is repr(C, packed) and
        // read_unaligned handles any alignment. We check s_magic immediately.
        let sb = unsafe { core::ptr::read_unaligned(buf.as_ptr() as *const types::Superblock) };
    ...
    pub(crate) fn read_inode(&self, inode_num: u32) -> Result<Inode, ()> {
        ...
        // SAFETY: gd_raw is a sector-aligned buffer containing group descriptors.
        // Each descriptor is 32 bytes. group < group_count is guaranteed by
        // inode_group() using the inodes_per_group from the superblock.
        let gd = unsafe { &*(gd_ptr.add(group as usize * 32) as *const types::GroupDescriptor) };
        ...
        self.device
            ...
            .map_err(|_| ())?;
        // SAFETY: sector_buf is 512 bytes read from the sector containing the
        // inode table entry. sec_off < 512 (computed from block_size and inode_size).
        // Inode is repr(C, packed) so read_unaligned is correct.
        Ok(unsafe {
            core::ptr::read_unaligned(sector_buf.as_ptr().add(sec_off as usize) as *const Inode)
        })
    }

    pub(crate) fn write_inode(&self, inode_num: u32, inode: &Inode) -> Result<(), ()> {
        let (group, index) = self.inode_group(inode_num);
        let gd_raw = self.read_gd_raw()?;
        let gd_ptr = gd_raw.as_ptr();
        // SAFETY: same as read_inode — gd_raw is sector-aligned, group index valid.
        let gd = unsafe { &*(gd_ptr.add(group as usize * 32) as *const types::GroupDescriptor) };
        let table_block = gd.bg_inode_table;
        let offset = index as u64 * self.inode_size as u64;
        let sec = (table_block as u64 * self.block_size as u64 + offset) / 512;
        ...
        self.device
            ...
            .read_sector(sec, &mut sector_buf)
            .map_err(|_| ())?;
        // SAFETY: sector_buf is 512 bytes. sec_off < 512 and Inode fits within
        // the sector (inode_size is at most 256). write_unaligned handles alignment.
        unsafe {
    ...
    pub(crate) fn write_block(&self, block_num: u32, data: &[u8]) -> Result<(), ()> {
        let sector = block_num as u64 * self.block_size as u64 / 512;
    ...
    pub(crate) fn now(&self) -> u32 {
        crate::get_ticks() as u32
    }

    // ── indirect block mapping ───────────────────────────────────────

    pub(crate) fn read_all_block_indices(&self, inode: &Inode) -> Result<Vec<u32>, ()> {
    ...
    fn read_indirect(&self, block_num: u32, level: u32) -> Result<Vec<u32>, ()> {
        ...
        // SAFETY: buf is a block_size-byte buffer read from disk. It was
        // allocated with block_size bytes. entries = block_size/4, and we
        // reinterpreted the bytes as u32 words — valid for on-disk ext2
    ...
    pub(crate) fn set_block_ptr(
        ...
        if level == 1 {
            ...
            // SAFETY: buf is a freshly read block of block_size bytes.
            // idx < entries (= block_size/4), so the write is in bounds.
            unsafe {
            ...
            // SAFETY: buf is block_size bytes, sub_idx < entries (= block_size/4).
            ...
            // SAFETY: buf2 is block_size bytes, sub_idx < entries (= block_size/4).
            unsafe {
                *(buf2.as_mut_ptr() as *mut u32).add(sub_idx) = sub;
            }
            fs.device
                .lock()
                .write_sector(*start_block as u64 * fs.block_size as u64 / 512, &buf2)
                .map_err(|_| ())
        }
    }

    pub(crate) fn write_file_blocks(&self, inode: &mut Inode, data: &[u8]) -> Result<(), ()> {
    ...
    pub(crate) fn free_all_blocks(&self, inode: &Inode) -> Result<(), ()> {
    ...
    pub(crate) fn free_indirect(&self, block_num: u32, level: u32) -> Result<(), ()> {
