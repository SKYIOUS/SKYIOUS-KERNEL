...
use super::types::{GroupDescriptor, Superblock};
...
impl Ext2FileSystem {
    ...
    pub(crate) fn read_raw_sb(&self) -> Result<Superblock, ()> {
        ...
        // SAFETY: buf is 1024 bytes read from sector 2 (superblock location).
        // Superblock is repr(C, packed) — any alignment is fine for read_unaligned.
        Ok(unsafe { core::ptr::read_unaligned(buf.as_ptr() as *const Superblock) })
    }

    pub(crate) fn write_raw_sb(&self, sb: &Superblock) -> Result<(), ()> {
        let mut buf = [0u8; 1024];
        // SAFETY: buf is a local 1024-byte stack buffer. write_unaligned handles
        // alignment. Superblock is repr(C, packed) — any bit pattern is valid.
        unsafe {
    ...
    fn write_gd_raw(&self, raw: &[u8]) -> Result<(), ()> {
        self.device
            .lock()
            .write_sector(self.gd_sector(), raw)
            .map_err(|_| ())
    }

    fn gd_ptr<'a>(raw: &'a mut [u8], group: u32) -> &'a mut GroupDescriptor {
        let off = group as usize * 32;
        // SAFETY: raw is a sector-sized buffer containing group descriptors.
        // Each GroupDescriptor is 32 bytes. group index is validated by the
        // caller (must be < group_count from the superblock).
    ...
    pub(crate) fn allocate_block(&self) -> Result<u32, ()> {
        ...
        let group_count = sb.s_blocks_count.div_ceil(self.blocks_per_group);
    ...
    pub(crate) fn allocate_inode(&self) -> Result<u32, ()> {
        let sb = self.read_raw_sb()?;
        let group_count = sb.s_inodes_count.div_ceil(self.inodes_per_group);
