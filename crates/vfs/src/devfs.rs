use crate::{FileSystem, Stat, VfsNode};
use alloc::string::String;
use alloc::sync::Arc;
use alloc::vec;
use alloc::vec::Vec;
use crossbeam_queue::ArrayQueue;
use vahi_drivers::input::InputEvent;
...
impl VfsNode for DevNode {
        ...
        match &self.inner {
            ...
            DevNodeInner::Random => {
                ...
                while buf.len() < max_len {
                    let w = vahi_crypto::GLOBAL_ENTROPY.get_u64().to_le_bytes();
                    buf.extend_from_slice(&w[..core::cmp::min(8, max_len - buf.len())]);
                }
                Ok(buf)
            }
            DevNodeInner::Framebuffer => {
                let h = vahi_drivers::graphics::HEIGHT.load(core::sync::atomic::Ordering::Relaxed);
                let stride =
                    vahi_drivers::graphics::STRIDE.load(core::sync::atomic::Ordering::Relaxed);
                let size = h * stride * 4;
                let mut buf = vec![0u8; size];
                let ptr =
                    vahi_drivers::graphics::FRAMEBUFFER.load(core::sync::atomic::Ordering::Relaxed);
                if !ptr.is_null() {
                    // SAFETY: ptr is a valid framebuffer address loaded from the
                    // FRAMEBUFFER atomic, which is initialized by the graphics driver
                    // during boot. size = height * stride * 4 (bytes per pixel),
                    // and buf was allocated with exactly that capacity.
                    unsafe {
            ...
            DevNodeInner::InputEvent(queue) => {
                ...
                for _ in 0..max_events {
                    if let Some(ev) = queue.pop() {
                        // SAFETY: InputEvent is repr(C) and contains only
                        // plain-old-data fields. Reinterpreting its bytes is safe.
                        let bytes = unsafe {
        ...
        match &self.inner {
            ...
            DevNodeInner::Framebuffer => {
                ...
                let ptr =
                    vahi_drivers::graphics::FRAMEBUFFER.load(core::sync::atomic::Ordering::Relaxed);
                if !ptr.is_null() {
                    // SAFETY: ptr is a valid framebuffer address from FRAMEBUFFER atomic.
                    // len is clamped to min(data.len(), fb_size) so we never write past
                    // the mapped framebuffer region.
                    unsafe {
            ...
            DevNodeInner::Tty0 => {
                // Skip framebuffer write — the fb pages may not be mapped in
                // exec'd process address spaces after a fresh AddressSpace::new().
                // Only write to serial, which is always accessible.
                for &b in data {
                    crate::vfs_serial_putc(b);
                }
                Ok(())
            }
            ...
            DevNodeInner::Speaker => {
                if data.len() >= 8 {
                    ...
                    vahi_drivers::audio::pcspeaker::beep(freq, dur);
                }
                Ok(())
            }
            DevNodeInner::BlockDevice(_) => Err(()),
    ...
    fn statfs(&self) -> Result<crate::StatFs, ()> {
        Ok(crate::StatFs {
            f_type: 0x01021994,
            f_bsize: 4096,
            f_blocks: 0,
            ...
            DevNodeInner::BlockDevice(idx) => {
                ...
                match request {
                    BLKGETSIZE64 => {
                        ...
                        // SAFETY: argp is a validated userspace pointer from the
                        // ioctl syscall dispatcher. copy_to_user handles permission
                        // checks and copies 8 bytes (u64 le).
                    ...
                    BLKSSZGET => {
                        ...
                        // SAFETY: argp is a validated userspace pointer from the
                        // ioctl syscall dispatcher.
                        if unsafe { user_access::copy_to_user(argp, &sector_size.to_le_bytes()) }
                            .is_err()
                        {
                            return Err(());
                        }
                    ...
                    BLKRD_SEC => {
                        ...
                        // SAFETY: argp is a validated userspace pointer.
                        if unsafe { user_access::copy_from_user(&mut op_buf, argp) }.is_err() {
                            return Err(());
                        }
                        // SAFETY: op_buf is size_of::<BlkIoctlOp>() bytes, and
                        // BlkIoctlOp is repr(C, packed) — any bit pattern is valid.
                        let op: BlkIoctlOp = unsafe {
                            core::ptr::read_unaligned(op_buf.as_ptr() as *const BlkIoctlOp)
                        };
                        if op.count > 1024 {
                            return Err(());
                        }
                        ...
                        for j in 0..op.count {
                            ...
                            if dev
                                .read_sector(op.sector + j, &mut buf[offset..offset + 512])
                                .is_err()
                            {
                                return Err(());
                            }
                        }
                        // SAFETY: op.buf is a validated userspace pointer to a buffer
                        // of at least data_size bytes, validated by the ioctl handler.
                    ...
                    BLKWR_SEC => {
                        ...
                        // SAFETY: op.buf is a validated userspace pointer to a buffer
                        // of at least data_size bytes, validated by the ioctl handler.
                        if unsafe { user_access::copy_from_user(&mut buf, op.buf as *const u8) }
                            .is_err()
                        {
                            return Err(());
                        }
                        for j in 0..op.count {
                            let offset = (j * 512) as usize;
                            if dev
                                .write_sector(op.sector + j, &buf[offset..offset + 512])
                                .is_err()
                            {
                                return Err(());
                            }
    ...
    fn children(&self) -> Result<Vec<Arc<dyn VfsNode>>, ()> {
        ...
        Ok(children
            .iter()
            .map(|c| c.clone() as Arc<dyn VfsNode>)
            .collect())
    }
...
impl DevFs {
    ...
    pub fn new() -> Self {
        ...
        let event0 = Arc::new(DevNode {
            ...
            inner: DevNodeInner::InputEvent(&vahi_drivers::input::KEYBOARD_EVENTS),
            children: Mutex::new(Vec::new()),
        });
        let event1 = Arc::new(DevNode {
            name: String::from("event1"),
            inner: DevNodeInner::InputEvent(&vahi_drivers::input::MOUSE_EVENTS),
            children: Mutex::new(Vec::new()),
        });
