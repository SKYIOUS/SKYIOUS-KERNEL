use crate::{get_ticks, FileSystem, Stat, VfsNode, S_IFDIR, S_IFREG};
    ...
    pub fn new() -> Self {
        ...
        add_child(
            ...
            file_fn(|| {
                ...
                out.push_str("No process table (vahi-task dependency removed)\n");
                out.into_bytes()
            }),
        );
        ...
        let sys_dir = add_child(&root, "sys", CtlInner::Dir);
        {
            ...
            let cpu_dir = add_child(&sys_dir, "cpu", CtlInner::Dir);
            {
                ...
                for cpu_idx in 0..total_cpus {
                    ...
                    add_child(
                        &dir,
                        "cpu_id",
                        file_fn(move || alloc::format!("{}\n", cid).into_bytes()),
                    );
                    add_child(
                        &dir,
                        "freq",
                        file_fn(|| Vec::from("~2500 MHz (LAPIC timer estimate)\n")),
                    );
                    add_child(
                        &dir,
                        "model",
                        file_fn(|| Vec::from("x86_64, QEMU Virtual CPU\n")),
                    );
                    add_child(
                        &dir,
                        "load",
                        file_fn(|| {
                            let ticks = get_ticks();
                            let idle = vahi_syscalls::get_per_cpu().idle_count;
                    ...
                    add_child(&dir, "current_task", file_fn(move || Vec::from("idle\n")));
                }
            }
            add_child(
                &cpu_dir,
            ...
            add_child(
                ...
                file_fn(|| {
                    ...
                    for i in 0..total {
                        out.push_str(&alloc::format!("  CPU{}: idle\n", i));
                    }
                    out.into_bytes()
                }),
            );
            ...
            add_child(
                ...
                "free",
                file_fn(|| {
                    let free = vahi_memory::buddy::BUDDY_ALLOCATOR
                        .lock()
                        .count_free_pages();
                    alloc::format!("{} pages\n", free).into_bytes()
                }),
            ...
            add_child(
                &mem_dir,
                "cached",
                file_fn(|| Vec::from("0 pages (no disk cache tracking)\n")),
            );
            ...
            #[cfg(feature = "net")]
            {
                ...
                add_child(
                    ...
                    "addr",
                    file_fn(|| {
                        let iface_lock = vahi_net::NETWORK_INTERFACE.lock();
                ...
                add_child(
                    ...
                    "hwaddr",
                    file_fn(|| {
                        let nic = vahi_drivers::net::NIC.lock();
                ...
                add_child(
                    ...
                    "stat",
                    file_fn(|| {
                        let sockets = vahi_net::SOCKETS.lock();
        ...
        add_child(
            &kernel_dir,
            "version",
            file_fn(|| Vec::from("SARGA OS — Vahi Kernel v0.3.0 — x86_64, Rust nightly\n")),
        );
        add_child(
            &kernel_dir,
            "uptime",
        ...
        add_child(&kernel_dir, "hostname", file_fn(|| Vec::from("sarga-os\n")));
        add_child(
            &kernel_dir,
            "log",
            file_fn(|| {
