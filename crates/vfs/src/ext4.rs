...
impl Ext4FileSystem {
    ...
    fn read_group_desc(&self, group: u32) -> Result<(u64, u64, u64), ()> {
        ...
        self.device
            .lock()
            .read_sector(self.block_to_sector(block_num), &mut buf)
            .map_err(|_| ())?;
        ...
        let (bitmap, bmap, itable) = if self.desc_size >= 64 {
            ...
            (
                entry.bg_block_bitmap as u64,
                entry.bg_inode_bitmap as u64,
                entry.bg_inode_table as u64,
            )
        };
    ...
    fn read_inode_raw(&self, inode_num: u32, buf: &mut [u8]) -> Result<(), ()> {
        ...
        buf[..copy_len]
            .copy_from_slice(&sector_buf[sector_off as usize..sector_off as usize + copy_len]);
        Ok(())
    }
    ...
    fn walk_extent_tree(
        ...
        if header.eh_depth == 0 {
            ...
            for idx in entries {
                ...
                self.device
                    .lock()
                    .read_sector(self.block_to_sector(child), &mut child_buf)
                    .map_err(|_| ())?;
    ...
    fn read_block(&self, phys_block: u64, buf: &mut [u8]) -> Result<(), ()> {
        ...
        for i in 0..sectors_per_block {
            ...
            self.device
                .lock()
                .read_sector(sector + i as u64, &mut sector_buf)
                .map_err(|_| ())?;
    ...
    pub fn self_test(device: &Arc<Mutex<dyn BlockDevice>>) -> Result<(), ()> {
        ...
        if (flags & EXT4_INODE_FLAG_EXTENTS) == 0 {
            crate::vfs_serial_write("[ext4] self-test WARN: root inode missing EXTENTS flag\n");
        }
        let extents = fsl.collect_extents(&raw)?;
        if extents.is_empty() {
            crate::vfs_serial_write("[ext4] self-test: empty root dir\n");
        } else {
            crate::vfs_serial_write(&alloc::format!(
                "[ext4] self-test: {} extents\n",
                extents.len()
            ));
        }
        Ok(())
    ...
    pub fn benchmark_read(&self, ino: u32, len: usize) -> Result<(), ()> {
        ...
        crate::vfs_serial_write(&alloc::format!(
            "[ext4] bench ino={} size={} {} ticks rate={} B/Mticks\n",
            ino,
            data.len(),
            elapsed,
            rate
        ...
        if has_extents {
            if let Ok(extents) = fs.collect_extents(&self.raw) {
                for (phys, _log, len) in &extents {
                    let mut block_data = vec![0u8; fs.block_size];
                    for bi in 0..*len as usize {
                        if fs.read_block(phys + bi as u64, &mut block_data).is_err() {
                            break;
                        }
                        let remaining = file_size - data.len();
                        let to_copy = core::cmp::min(remaining, fs.block_size);
