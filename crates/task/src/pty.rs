//! PTY (pseudo-terminal) pair implementation.
//!
//! Provides bidirectional byte pipes between a master (shell) and slave (program)
//! end, with optional canonical line discipline on the slave read path.
...
impl PtyEnd {
    pub fn new() -> Self {
        PtyEnd {
            buf: ArrayQueue::new(PTY_BUF_SIZE),
            peer_closed: false,
        }
    }
}
...
pub fn pty_read_master(pair: &Arc<Mutex<PtyPair>>, buf: &mut [u8]) -> Result<usize, ()> {
    ...
    if count == 0 && p.master.peer_closed {
        return Err(());
    }
    Ok(count)
}
...
pub fn pty_read_slave(
    ...
    if ldisc.canonical {
        ...
        if count == 0 && p.slave.peer_closed {
            return Err(());
        }
        Ok(count)
    } else {
