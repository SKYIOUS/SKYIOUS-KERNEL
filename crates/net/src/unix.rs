...
pub fn connect_unix(handle: u64, addr_ptr: *const u8, addrlen: u64) -> Result<(), Errno> {
    ...
    if sock_type == UnixSocketType::Stream {
        vahi_types::wake_pipe(target_handle);
    }
    Ok(())
}
...
pub fn accept_unix(handle: u64, addr_ptr: *mut u8, addrlen_ptr: *mut u32) -> Result<u64, Errno> {
    ...
    loop {
        if vahi_syscalls::check_signal_interrupt() {
        ...
        vahi_types::block_on_pipe(handle);
    }
...
pub fn sendto_unix(
    ...
    let is_dgram_with_addr;
    {
        ...
        is_dgram_with_addr =
            inner.sock_type == UnixSocketType::Dgram && !addr_ptr.is_null() && addrlen >= 2;
...
pub fn getpeercred_unix(handle: u64) -> Result<(u32, u32, u32), Errno> {
    ...
    // Peer creds resolve through the registered process provider (kernel-owned
    // per-CPU identity), not a crate-local process table shadow.
    let (pid, uid, gid) = vahi_types::current_peer_creds().unwrap_or((0, 0, 0));
    Ok((pid, uid, gid))
}
    ...
    if let Some(sock) = socks.remove(&handle) {
        ...
        if let Some(ph) = peer_handle {
            if let Some(peer) = socks.get(&ph) {
                ...
                vahi_types::wake_pipe(ph);
            }
        }
        vahi_types::wake_pipe(handle);
    }
}
...
pub fn sys_socketpair(domain: u64, type_: u64, _protocol: u64, sv: *mut i32) -> Result<u64, Errno> {
    ...
    // Stub: the real implementation in kernel/src/syscalls/ accesses
    // CURRENT_PROCESS to install FDs into the process fd table.
    // In the standalone crate, return the handle pair as raw indices.
    let fd1 = h1 as u32;
    let fd2 = h2 as u32;
    ...
    unsafe {
        user_access::copy_to_user(
            sv as *mut u8,
            core::slice::from_raw_parts(&arr as *const i32 as *const u8, 8),
        )
