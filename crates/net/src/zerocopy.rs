...
impl ScatterGatherIo {
    ...
    pub fn from_iovec(
        ...
        let iov_size = core::mem::size_of::<vahi_syscalls::net_helpers::iovec>();
        let mut iov_buf: Vec<vahi_syscalls::net_helpers::iovec> = Vec::with_capacity(iov_count);
        for _ in 0..iov_count {
            iov_buf.push(vahi_syscalls::net_helpers::iovec {
        ...
        unsafe {
            if vahi_syscalls::user_access::copy_from_user(
    ...
    fn drop(&mut self) {
        ...
        for seg in &self.segments {
            if !seg.is_zerocopy && !seg.data.is_null() {
                unsafe {
                    let _ = Vec::from_raw_parts(seg.data as *mut u8, seg.len, seg.len);
                }
            }
        }
    }
...
pub fn scatter_gather_recv(
    recv_data: &[u8],
    iovecs: &[vahi_syscalls::net_helpers::iovec],
) -> usize {
    let mut offset = 0;
...
pub fn iovec_total_capacity(iovecs: &[vahi_syscalls::net_helpers::iovec]) -> usize {
...
mod tests {
    ...
    fn test_zerocopy_overlap_detection() {
        ...
        // addr2 starts at +2048, well within addr1's 4096-byte range
        let addr2 = 0x3000_0800usize;
    ...
    fn test_zerocopy_invalid_buffer() {
        assert_eq!(
            register_zerocopy_buffer(0, 4096, 1),
            ZerocopyResult::InvalidBuffer
        );
        assert_eq!(
            register_zerocopy_buffer(0x1000, 0, 1),
            ZerocopyResult::InvalidBuffer
        );
    }
