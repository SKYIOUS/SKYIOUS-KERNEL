use crate::graphics::{FRAMEBUFFER, HEIGHT, STRIDE, WIDTH};
use alloc::vec::Vec;
use core::fmt;
...
impl ConsoleWriter {
    ...
    fn scroll(&mut self) {
        ...
        if !fb_ptr.is_null() && self.rows > 1 {
            let h = crate::graphics::HEIGHT.load(core::sync::atomic::Ordering::Relaxed);
            unsafe {
                core::ptr::copy(fb_ptr.add(stride * 16), fb_ptr, (h - 16) * stride);
    ...
    fn write_char_at(&mut self, cx: usize, cy: usize, c: char, fg: u32, bg: u32) {
        ...
        if fb_ptr.is_null() || cx >= self.cols || cy >= self.rows {
            return;
        }
        ...
        if buf[1] == b'[' {
            ...
            if let Some(cmd) = command {
                match cmd {
                    ...
                    b'J' => {
                        let mode = if !params.is_empty() && params[0] >= 0 {
                            params[0]
                        } else {
                            0
                        };
                    ...
                    b'h' if !params.is_empty() && params[0] == -2 => {
                        self.cursor_visible = true;
                    }
                    b'l' if !params.is_empty() && params[0] == -2 => {
    ...
    pub fn clear_screen(&mut self) {
        ...
        let height = HEIGHT.load(Ordering::Relaxed);
        let stride = STRIDE.load(Ordering::Relaxed);
        if fb_ptr.is_null() || height == 0 || stride == 0 {
            return;
        }

        // ponytail: STOSD-equivalent row fill avoids per-pixel bounds checks.
        // Per-pixel write_volatile is ~100x slower under QEMU TCG.
        let bg = self.background;
        unsafe {
            for y in 0..height {
                let row = fb_ptr.add(y * stride);
                let mut x = 0;
                while x < stride {
                    *row.add(x) = bg;
                    x += 1;
