...
// SAFETY CONTRACT:
// HBA registers are accessed via MMIO (PCI BAR0 + offset). Codec registers
// are accessed via verb commands sent through the CORB (Command Output Ring
// Buffer) and parsed from the RIRB (Response Input Ring Buffer). DMA buffers
// for audio streams (BDL — Buffer Descriptor List) are allocated via Box::leak
// with physical addresses written to the controller's SDnBAS/SDnLVI registers.
use alloc::vec::Vec;
use core::ptr;
...
impl HdaController {
    ...
    fn send_verb(&mut self, codec_addr: u8, nid: u8, verb: u16, param: u16) -> u32 {
        ...
        if let Some((ptr, _)) = self.corb_buf.as_ref() {
            unsafe {
                core::arch::x86_64::_mm_clflush((ptr.add(wp) as *const u32) as *const u8);
            }
        }
        ...
        hda_println!(
            "HDA: RIRB timeout after verb 0x{:08X} (start_rp={})",
            verb_data,
            start_rp
        );
        0
    ...
    pub fn init(&mut self) {
        ...
        let (_vmaj, _vmin, num_out) = {
            ...
            hda_println!(
                "HDA: Controller at 0x{:x}, Version {}.{}",
                base_addr,
                vmaj,
                vmin
            );
        ...
        unsafe {
            ptr::write_bytes(corb_ptr, 0, CORB_SIZE * 4);
        }
        unsafe {
            ptr::write_bytes(rirb_ptr, 0, RIRB_SIZE * 4);
        }
        self.corb_buf = Some((corb_ptr as *mut u32, CORB_SIZE));
        self.rirb_buf = Some((rirb_ptr as *mut u32, RIRB_SIZE));

        let corb_phys = vahi_memory::virt_to_phys(VirtAddr::from_ptr(corb_ptr)).unwrap();
        let rirb_phys = vahi_memory::virt_to_phys(VirtAddr::from_ptr(rirb_ptr)).unwrap();
        hda_println!(
        ...
        for addr in 0..4 {
            ...
            hda_println!(
                "HDA: Codec {}: 0x{:04X}:0x{:04X}, rev 0x{:02X}",
                addr,
                vendor_upper,
                vendor_lower,
                rev
        ...
        if out_nid == 0 {
            hda_println!("HDA: No output converter found");
            return;
        }
        if pin_nid == 0 {
            hda_println!("HDA: No pin complex found");
            return;
        }

        hda_println!(
            "HDA: AFG=0x{:02X}, Output=0x{:02X}, Pin=0x{:02X}",
            afg_nid,
            out_nid,
            pin_nid
        );
        ...
        let sine_table: [i16; 256] = [
            ...
            11793, 12540, 13279, 14010, 14732, 15446, 16151, 16846, 17531, 18205, 18868, 19520,
            20160, 20788, 21403, 22005, 22595, 23170, 23732, 24279, 24812, 25330, 25833, 26320,
            26791, 27246, 27684, 28106, 28511, 28899, 29269, 29622, 29957, 30274, 30572, 30852,
            31114, 31357, 31581, 31786, 31972, 32138, 32285, 32413, 32522, 32610, 32679, 32729,
            32758, -32768, -32758, -32729, -32679, -32610, -32522, -32413, -32285, -32138, -31972,
            -31786, -31581, -31357, -31114, -30852, -30572, -30274, -29957, -29622, -29269, -28899,
            -28511, -28106, -27684, -27246, -26791, -26320, -25833, -25330, -24812, -24279, -23732,
            -23170, -22595, -22005, -21403, -20788, -20160, -19520, -18868, -18205, -17531, -16846,
            -16151, -15446, -14732, -14010, -13279, -12540, -11793, -11039, -10278, -9512, -8739,
            -7962, -7179, -6393, -5602, -4808, -4011, -3212, -2410, -1608, -804, 0, 804, 1608,
            2410, 3212, 4011, 4808, 5602, 6393, 7179, 7962, 8739, 9512, 10278, 11039, 11793, 12540,
            13279, 14010, 14732, 15446, 16151, 16846, 17531, 18205, 18868, 19520, 20160, 20788,
            21403, 22005, 22595, 23170, 23732, 24279, 24812, 25330, 25833, 26320, 26791, 27246,
            27684, 28106, 28511, 28899, 29269, 29622, 29957, 30274, 30572, 30852, 31114, 31357,
            31581, 31786, 31972, 32138, 32285, 32413, 32522, 32610, 32679, 32729, 32758, -32768,
            -32758, -32729, -32679, -32610, -32522, -32413, -32285, -32138, -31972, -31786, -31581,
            -31357, -31114, -30852, -30572, -30274, -29957, -29622, -29269, -28899, -28511, -28106,
            -27684, -27246, -26791, -26320, -25833, -25330, -24812, -24279, -23732, -23170, -22595,
            -22005, -21403, -20788, -20160, -19520, -18868, -18205, -17531, -16846, -16151, -15446,
            -14732, -14010, -13279, -12540, -11793, -11039, -10278, -9512, -8739, -7962, -7179,
    ...
    fn setup_output_stream(&mut self, index: usize, data: &[u8]) {
        ...
        let bdl = unsafe {
            let ptr =
                alloc::alloc::alloc(alloc::alloc::Layout::from_size_align(bdl_size, 128).unwrap());
            if ptr.is_null() {
                return;
            }
        ...
        for i in 0..bdl_entries {
            ...
            let buf_phys = vahi_memory::virt_to_phys(VirtAddr::from_ptr(buf)).unwrap();
        ...
        let bdl_phys = vahi_memory::virt_to_phys(VirtAddr::from_ptr(bdl.as_ptr())).unwrap();
        ...
        hda_println!(
            "HDA: Stream {} started: tag=1, {}Hz, {}ch, {}bit",
            index,
            SAMPLE_RATE,
            CHANNELS,
            BITS_PER_SAMPLE
    ...
    pub fn set_volume(&mut self, percent: u8) {
        let out = match self.out_nid {
            Some(n) => n,
            None => return,
        };
        let afg = match self.afg_nid {
            Some(n) => n,
            None => return,
        };
        ...
        let vol_linear = if percent >= 100 {
            0x7F
        } else {
            (percent as u16 * 127 / 100) as u8
        };
        ...
        hda_println!(
            "HDA: Volume set to {}% (amp val 0x{:02X})",
            percent,
            amp_val
        );
    }
