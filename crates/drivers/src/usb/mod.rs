...
use crate::usb::xhci::XhciController;
...
pub extern "C" fn usb_hid_poller() -> ! {
    ...
    loop {
        ...
        // Sleep ~1 tick via the registered sleep facade (the kernel's live
        // scheduler marks the thread Blocked with a deadline and yields).
        let target_tick = crate::get_ticks() + 1;
        vahi_types::sleep_until_tick(target_tick);
    }
}
