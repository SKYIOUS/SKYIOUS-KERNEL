// SAFETY CONTRACT:
// UHCI uses I/O ports (base_addr + offset) for frame list, QH/TD rings,
// and port status/control. DMA frame lists and transfer descriptors are
// allocated via DMA-alloc functions with physical addresses written to
// the UHCI frame base address register. Port resets are timed per spec.
use vahi_hal::dma::DmaBuf;
...
impl UhciController {
    pub fn new(io_base: u16) -> Self {
        ...
        let td_phys = vahi_memory::virt_to_phys_dma(VirtAddr::new(ptr as u64)).as_u64() as u32;
    ...
    // ─── Port I/O helpers ───────────────────────────────────────────────
    fn rw(&self, r: u16) -> u16 {
        unsafe { Port::new(self.io + r).read() }
    }
    fn ww(&mut self, r: u16, v: u16) {
        unsafe {
            Port::new(self.io + r).write(v);
        }
    }
    fn wl(&mut self, r: u16, v: u32) {
        self.ww(r, v as u16);
        self.ww(r + 2, (v >> 16) as u16);
    }
    fn wb(&mut self, r: u16, v: u8) {
        unsafe {
            Port::new(self.io + r).write(v);
        }
    }

    fn portsc(&self, p: usize) -> u16 {
        self.rw(PORTSC1 + (p as u16 * 2))
    }
    fn set_portsc(&mut self, p: usize, v: u16) {
        self.ww(PORTSC1 + (p as u16 * 2), v);
    }

    // ─── TD allocator ────────────────────────────────────────────────────
    fn td_alloc(&mut self) -> Option<(*mut Td, u32)> {
        if self.td_free >= TD_POOL {
            return None;
        }
        let i = self.td_free;
        self.td_free += 1;
    ...
    pub fn init(&mut self) {
        ...
        let mut t = 0u32;
        while (self.rw(USBSTS) & STS_HCHALTED) != 0 {
            core::hint::spin_loop();
            t += 1;
            if t > 2000 {
                break;
    ...
    fn enumerate(&mut self, port: usize) {
        ...
        let mut t = 0u32;
        while self.portsc(port) & PORT_RESET_MASK != 0 {
            core::hint::spin_loop();
            t += 1;
            if t > 100000 {
                break;
        ...
        if !self.ctrl_in(1, 0x80, 6, 0x0100, 0, &mut dev_desc2, low) {
            return;
        }
        ...
        if !self.ctrl_in(1, 0x80, 6, 0x0200, 0, &mut cfg_hdr, low) {
            return;
        }
        ...
        if !self.ctrl_in(1, 0x80, 6, 0x0200, 0, &mut cfg_buf, low) {
            return;
        }
        ...
        while off + 3 < cfg_len && off < cfg_total {
            ...
            if dtype == 4 && off + 9 <= cfg_len {
                ...
                crate::serial_write(&alloc::format!(
                    "[UHCI]   iface class={:02x} sub={:02x} proto={:02x}\n",
                    if_class,
                    if_sub,
                    if_proto
                ));
    ...
    fn ctrl_in(
        ...
        let (td0, ph0) = match self.td_alloc() {
            Some(v) => v,
            None => return false,
        };
        let (td1, ph1) = match self.td_alloc() {
            Some(v) => v,
            None => return false,
        };
        let (td2, ph2) = match self.td_alloc() {
            Some(v) => v,
            None => return false,
        };
        ...
        if ok && len > 0 {
            unsafe {
                core::ptr::copy_nonoverlapping(self.data[1].virt(), data.as_mut_ptr(), len);
            }
        }
        self.td_free_all();
        unsafe {
