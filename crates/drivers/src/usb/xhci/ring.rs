...
use super::regs::{
    trb_type, XhciDeviceContext, XhciTrb, CYCLE, LINK_TOGGLE_CYCLE, MAX_ENDPOINTS, TRB_LINK,
};
...
impl TransferRing {
    ...
    /// `bdf` is the PCI Bus/Device/Function of the xHCI controller, used for IOMMU DMA mapping.
    pub fn new(bdf: u16) -> Option<Self> {
        let buf = DmaBuf::new(RING_SIZE * 16)?;
        let base = buf.virt() as *mut XhciTrb;
        let phys = buf.phys();
        // Map the transfer ring through IOMMU so the xHCI controller can DMA to it.
...
impl Slot {
    pub fn new() -> Self {
        ...
        Slot {
            device_ctx: XhciDeviceContext::zeroed(),
            rings,
        }
    }
}
