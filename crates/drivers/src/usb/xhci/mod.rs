...
// SAFETY CONTRACT:
// xHCI MMIO registers are at PCI BAR0. Capability/correctness registers are
// read during init to determine max_slots, doorbell offset, and runtime regs.
// Transfer rings (TRBs) are allocated via Box::leak with physical addresses
// in the dequeue pointer and endpoint context. Command ring is a single
// contiguous allocation. Event ring uses a separate DMA buffer with ERDP/ERST.
...
use crate::usb::core::UsbHostController;
use crate::usb::{hid, HidEndpoint};

use regs::*;
use ring::{Slot, TransferRing, RING_SIZE};
...
impl XhciController {
    pub fn new(base_addr: usize, bdf: u16) -> Self {
        Self {
            base_addr,
            bdf,
            cap_length: 0,
    ...
    pub fn init(&mut self) {
        ...
        // Map DCBAAP through IOMMU so the xHCI controller can DMA to it.
        vahi_arch::iommu::iommu_map(
            self.bdf,
            dphys,
            dphys,
            (256 * 8) as u64,
        ...
        // Map command ring through IOMMU.
        vahi_arch::iommu::iommu_map(
            self.bdf,
            cphys,
            cphys,
        ...
        self.erst_base = erst;
        let erstphys = vahi_memory::virt_to_phys_dma(VirtAddr::new(erst as u64)).as_u64();
        // Map ERST through IOMMU.
        vahi_arch::iommu::iommu_map(self.bdf, erstphys, erstphys, 16u64, 0x3 /* RD | WR */);
    ...
    fn address_device(&mut self, slot_id: u8, port: usize, speed: u8) -> bool {
        ...
        let ring = match TransferRing::new(self.bdf) {
            Some(r) => r,
            None => return false,
        };
        ...
        let input_phys = {
            ...
            vahi_memory::virt_to_phys_dma(VirtAddr::new(p as u64)).as_u64()
        };
        ...
        let device_ctx_phys =
            vahi_memory::virt_to_phys_dma(VirtAddr::new(&*device_ctx as *const _ as u64)).as_u64();
        unsafe {
    ...
    fn configure_interrupt_in_endpoint(
        ...
        let ring = TransferRing::new(self.bdf)?;
        let dequeue = ring.enqueue_phys() | 1;

        let mut input = XhciInputContext::zeroed();
        let input_phys =
            vahi_memory::virt_to_phys_dma(VirtAddr::new(&*input as *const _ as *const u8 as u64))
                .as_u64();
    ...
    fn identify_device(&mut self, slot_id: u8) {
        ...
        if !self.control_transfer(
            slot_id,
            crate::usb::core::USB_DIR_IN | crate::usb::core::USB_TYPE_STANDARD,
            crate::usb::core::USB_REQ_GET_DESCRIPTOR,
            (crate::usb::core::USB_DESC_DEVICE as u16) << 8,
            0,
            &mut dev_desc,
        ) {
            // println removed during extraction
        ...
        if !self.control_transfer(
            ...
            crate::usb::core::USB_DIR_IN | crate::usb::core::USB_TYPE_STANDARD,
            crate::usb::core::USB_REQ_GET_DESCRIPTOR,
            (crate::usb::core::USB_DESC_CONFIG as u16) << 8,
            0,
            &mut cfg_hdr,
        ) {
            return;
        ...
        self.control_transfer(
            slot_id,
            crate::usb::core::USB_TYPE_STANDARD,
            crate::usb::core::USB_REQ_SET_CONFIGURATION,
            1,
            0,
            &mut [],
        );
    ...
    fn probe_interfaces(&mut self, slot_id: u8, cfg: &[u8]) {
        ...
        while off + 2 <= cfg.len() {
            ...
            let dtype = cfg[off + 1];
            match dtype {
                crate::usb::core::USB_DESC_INTERFACE => {
                    if off + 9 > cfg.len() {
                        break;
                    }
                    if cur_iface_class == crate::usb::core::USB_CLASS_HID {
                ...
                crate::usb::core::USB_DESC_ENDPOINT => {
                    if off + 7 > cfg.len() {
                        break;
                    }
                    let addr = cfg[off + 2];
                    ...
                    if is_in && transfer == crate::usb::core::USB_ENDPOINT_INTERRUPT {
    ...
    fn register_hid(
        ...
        let set_proto_ok = self.control_transfer(
            slot_id,
            crate::usb::core::USB_TYPE_CLASS | crate::usb::core::USB_DIR_OUT,
        ...
        if self
            .configure_interrupt_in_endpoint(slot_id, ep_addr, max_pkt, interval)
            .is_none()
        {
            // println removed during extraction
            return;
        ...
        // println removed during extraction
        crate::usb::register_hid_endpoint(HidEndpoint {
            kind,
            device_addr: slot_id,
            ep_addr,
            max_packet: max_pkt,
    ...
    fn submit_command(&mut self, trb: XhciTrb) -> Option<u64> {
        ...
        let trb_phys = unsafe {
            ...
            vahi_memory::virt_to_phys_dma(VirtAddr::new(slot as u64)).as_u64()
        };
    ...
    fn poll_event(&mut self) -> Option<XhciTrb> {
        ...
        let trb = unsafe {
            core::ptr::read_volatile(
                self.event_ring_base.add(self.event_ring_index) as *const XhciTrb
            )
        };
        ...
        if cycle_bit == producer_cycle {
            ...
            let erdp = vahi_memory::virt_to_phys_dma(VirtAddr::new(
...
// (via vahi_sync::IrqSafeMutex) disables interrupts across the critical section.
...
impl UsbHostController for XhciController {
    fn control_transfer(
        ...
        let data_trb_opt = if !data.is_empty() {
            if let Some(dma_buf) = PooledDma::alloc(data.len(), self.bdf) {
                let dma_phys = dma_buf.phys();
                if !dir_in {
                    unsafe {
                        core::ptr::copy_nonoverlapping(data.as_ptr(), dma_buf.virt(), data.len());
                    }
                }
        ...
        let status_trb = (0, 0, trb_type(TRB_STATUS_STAGE) | status_dir | IOC);
        ...
        if dir_in {
            if let Some(dma) = self.pending_dma.last() {
                unsafe {
                    core::ptr::copy_nonoverlapping(dma.virt(), data.as_mut_ptr(), data.len());
                }
            }
        }
        self.pending_dma.pop();
    ...
    fn interrupt_transfer(&mut self, device_addr: u8, endpoint_addr: u8, data: &mut [u8]) -> bool {
        let slot_id = device_addr;
        let ep_num = endpoint_addr & 0x0F;
        let dci = 2 * ep_num + 1;

        let dma_phys = if let Some(dma_buf) = PooledDma::alloc(data.len().max(1), self.bdf) {
        ...
        let td = vec![(dma_phys, data.len() as u32, trb_type(TRB_NORMAL) | IOC)];
        let last_trb_phys = match self.submit_td(slot_id, dci, &td) {
            Some(p) => p,
            None => {
                self.pending_dma.pop();
        ...
        if let Some(dma) = self.pending_dma.last() {
            unsafe {
                core::ptr::copy_nonoverlapping(dma.virt(), data.as_mut_ptr(), n);
            }
        }
        self.pending_dma.pop();
        true
