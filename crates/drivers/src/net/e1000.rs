// SAFETY CONTRACT:
// E1000 MMIO registers are accessed via Volatile reads/writes through
// PCI BAR0-mapped addresses. All register offsets follow the Intel E1000
// datasheet. DMA descriptor rings (RX/TX) are allocated via Box::leak with
// physical addresses written to the NIC's RDBAL/RDBAH/TDBAL/TDBAH registers.
// Interrupt handling reads ICR register and clears via write-1-to-clear.
use alloc::vec;
use alloc::vec::Vec;
...
impl E1000 {
    ...
    pub unsafe fn new(base_addr: usize, bdf: u16) -> Self {
        E1000 {
            base_addr,
            bdf,
            rx_descs: &mut [],
    ...
    fn init_rx(&mut self) -> &'static mut [RxDesc] {
        ...
        let desc_phys = vahi_memory::virt_to_phys(desc_virt).expect("RX Ring Phys failed");

        // Map descriptor ring through IOMMU so the NIC can DMA to it
        vahi_arch::iommu::iommu_map(
            self.bdf,
            desc_phys.as_u64(),
            desc_phys.as_u64(),
            (core::mem::size_of::<RxDesc>() * desc_count) as u64,
            0x3, /* RD | WR */
        );
        ...
        for desc in descs.iter_mut() {
            ...
            let buf_phys = vahi_memory::virt_to_phys(buf_virt).expect("RX Buf Phys failed");

            // Map each RX buffer through IOMMU
            vahi_arch::iommu::iommu_map(
                self.bdf,
                buf_phys.as_u64(),
                buf_phys.as_u64(),
    ...
    fn init_tx(&mut self) -> &'static mut [TxDesc] {
        ...
        let desc_ptr = descs.as_ptr();
        let desc_virt = VirtAddr::from_ptr(desc_ptr);
        let desc_phys = vahi_memory::virt_to_phys(desc_virt).expect("TX Ring Phys failed");

        // Map TX descriptor ring through IOMMU
        vahi_arch::iommu::iommu_map(
            self.bdf,
            desc_phys.as_u64(),
            desc_phys.as_u64(),
            (core::mem::size_of::<TxDesc>() * desc_count) as u64,
            0x3, /* RD | WR */
        );
        ...
        for desc in descs.iter_mut() {
            let buf: &'static mut [u8] = Box::leak(vec![0u8; 2048].into_boxed_slice());
            let buf_virt = VirtAddr::from_ptr(buf.as_ptr());
            let buf_phys = vahi_memory::virt_to_phys(buf_virt).expect("TX Buf Phys failed");

            // Map each TX buffer through IOMMU
            vahi_arch::iommu::iommu_map(
                self.bdf,
                buf_phys.as_u64(),
                buf_phys.as_u64(),
    ...
    pub fn send_packet(&mut self, data: &[u8]) {
        ...
        unsafe {
            core::ptr::write_unaligned(
                core::ptr::addr_of_mut!(self.tx_descs[cur].length),
                data.len() as u16,
            );
            core::ptr::write_unaligned(
                core::ptr::addr_of_mut!(self.tx_descs[cur].cmd),
                (1 << 0) | (1 << 1) | (1 << 3),
            );
        ...
        for _ in 0..TX_TIMEOUT {
            let s;
            unsafe {
                s = core::ptr::read_unaligned(core::ptr::addr_of!(self.tx_descs[cur].status));
            }
            if s & 1 != 0 {
                return;
            }
    ...
    fn reset_nic(&mut self) {
        ...
        if !self.rx_descs.is_empty() {
            let rx_phys =
                vahi_memory::virt_to_phys(x86_64::VirtAddr::from_ptr(self.rx_descs.as_ptr()))
                    .expect("RX ring phys");
        ...
        if !self.tx_descs.is_empty() {
            let tx_phys =
                vahi_memory::virt_to_phys(x86_64::VirtAddr::from_ptr(self.tx_descs.as_ptr()))
                    .expect("TX ring phys");
