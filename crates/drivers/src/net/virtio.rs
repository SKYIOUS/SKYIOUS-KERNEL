// SAFETY CONTRACT:
// All port I/O operations in this file access VirtIO device registers at
// fixed offsets from `base_addr`. The base_addr is obtained from PCI BAR1
// during device initialization and is valid for the kernel's lifetime.
// VirtIO spec mandates specific read/write sequences for initialization.
// DMA ring buffers are allocated via Box::leak (never freed) with physical
// addresses provided to the device via queue_pfn registers.
use alloc::sync::Arc;
use alloc::vec;
use alloc::vec::Vec;
...
impl VirtIOQueue {
    ...
    pub fn pfn(&self) -> u32 {
        ...
        let phys_addr = vahi_memory::virt_to_phys(virt_addr).expect("Virtq Phys failed");
...
impl VirtIONet {
    ...
    pub fn new(base_addr: u16) -> Self {
        ...
        unsafe {
            status_port.write(STATUS_ACK | STATUS_DRIVER);
        }
        ...
        unsafe {
            status_port.write(STATUS_ACK | STATUS_DRIVER | STATUS_DRIVER_OK);
        }
    ...
    fn populate_rx(&mut self) {
        ...
        for i in 0..self.rx_queue.size {
            ...
            let phys = vahi_memory::virt_to_phys(VirtAddr::from_ptr(buf.as_ptr())).unwrap();
    ...
    pub fn send_packet(&mut self, data: &[u8]) {
        ...
        let leaked_pkt = match self.tx_bufs.iter().position(|b| b.len() >= pkt_len) {
            ...
            None => {
                ...
                let len = leaked.len();
                // SAFETY: transmute extends the lifetime of the boxed slice.
                // The Box::leak'd memory is never freed, so 'static is correct.
                self.tx_bufs.push(unsafe {
                    core::mem::transmute::<&'static mut [u8], &'static mut [u8]>(leaked)
                });
                // SAFETY: ptr and len are from the leaked box, valid for 'static.
                unsafe { core::slice::from_raw_parts_mut(ptr, len) }
            }
        };

        let phys = vahi_memory::virt_to_phys(VirtAddr::from_ptr(leaked_pkt.as_ptr())).unwrap();
        ...
        self.tx_queue.available.ring
            [self.tx_queue.available.idx as usize % self.tx_queue.size as usize] = head;
        ...
        if last_used != current_used {
            ...
            unsafe {
                core::ptr::copy_nonoverlapping(
                    (virt_addr + 10) as *const u8,
                    data.as_mut_ptr(),
                    result_len,
                );
            }

            self.rx_queue.available.ring
                [self.rx_queue.available.idx as usize % self.rx_queue.size as usize] =
                desc_idx as u16;
