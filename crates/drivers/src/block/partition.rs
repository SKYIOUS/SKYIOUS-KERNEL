...
pub fn parse_mbr(device: &Arc<Mutex<dyn BlockDevice>>) -> Result<Vec<Partition>, ()> {
    ...
    for i in 0..4 {
        ...
        let entry: MbrPartitionEntry = unsafe {
            core::ptr::read_unaligned(mbr.as_ptr().add(offset) as *const MbrPartitionEntry)
        };
        if entry.type_ == 0 {
            continue;
        }
...
fn read_gpt_entries(
    ...
    for i in 0..num_entries as usize {
        ...
        if i % entries_per_sector == 0 {
            device
                .lock()
                .read_sector(sector_idx, &mut buf)
                .map_err(|_| ())?;
        }
        ...
        let _unique_guid =
            unsafe { core::ptr::read_unaligned(entry_ptr.add(16) as *const [u8; 16]) };
...
pub fn parse_gpt(device: &Arc<Mutex<dyn BlockDevice>>) -> Result<Vec<Partition>, ()> {
    ...
    let _revision = u32::from_le_bytes(
        header[8..12]
            .try_into()
            .expect("GPT header revision should be 4 bytes"),
    );
    let header_size = u32::from_le_bytes(
        header[12..16]
            .try_into()
            .expect("GPT header size should be 4 bytes"),
    );
    let entry_lba = u64::from_le_bytes(
        header[72..80]
            .try_into()
            .expect("GPT entry LBA should be 8 bytes"),
    );
    let num_entries = u32::from_le_bytes(
        header[80..84]
            .try_into()
            .expect("GPT num entries should be 4 bytes"),
    );
    let entry_size = u32::from_le_bytes(
        header[84..88]
            .try_into()
            .expect("GPT entry size should be 4 bytes"),
    );
...
impl BlockDevice for PartitionDevice {
    ...
    fn write_sector(&mut self, sector: u64, buf: &[u8]) -> Result<(), BlockDeviceError> {
        ...
        self.parent
            .lock()
            .write_sector(self.lba_start + sector, buf)
    }
