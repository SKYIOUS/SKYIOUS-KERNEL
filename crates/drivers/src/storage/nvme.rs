...
use alloc::boxed::Box;
use alloc::sync::Arc;
use vahi_hal::dma::{DmaBuf, PooledDma};
...
impl RingBuf {
    ...
    fn entry(&self, index: u32) -> *mut u8 {
        // SAFETY: buf is a DMA-allocated ring buffer of num_entries * entry_size
        // bytes. index % num_entries keeps us in bounds.
        unsafe {
            self.buf
                .as_ptr()
                .add((index % self.num_entries) as usize * self.entry_size) as *mut u8
        }
    }
}
...
pub struct NvmeController {
    ...
    /// PCI Bus/Device/Function — needed for IOMMU DMA mapping.
    bdf: u16,
    db_stride: u32,
    admin_sq: RingBuf,
    admin_cq: RingBuf,
...
impl NvmeController {
    ...
    pub fn new(
        ...
        // SAFETY: base_addr is from PCI BAR0, MMIO-mapped. NvmeRegisters
        // is the standard NVMe controller register layout.
        ...
        if (regs.cc.read() & CC_EN) != 0 {
            ...
            if !Self::wait_rdy(regs, false, timeout_ms) {
                return None;
            }
        }
        ...
        // Enable
        regs.cc
            .write(CC_EN | CC_IOCQES_16 | CC_IOSQES_64 | CC_MPS_4K);
        core::sync::atomic::fence(core::sync::atomic::Ordering::SeqCst);
        if !Self::wait_rdy(regs, true, timeout_ms) {
            return None;
        }
        ...
        let bdf = ((pci_bus as u16) << 8) | ((pci_slot as u16) << 3) | (pci_func as u16);
        ...
        if let Some(health) = ctrl.smart_health() {
            crate::println!(
                "NVMe: SMART health: temp={}C spare={}% used={}%",
        ...
        // SAFETY: ctrl is &'static mut NvmeController. The cast to *mut Self
        // creates a mutable alias — safe here because NvmeDisk does not exist
        // until this point, and only one NvmeDisk per controller is created.
        let d = NvmeDisk {
            ctrl: unsafe { &mut *(ctrl as *mut Self) },
        };
        register_block_device(Arc::new(Mutex::new(d)));

        crate::println!(
            "NVMe: ns {} ({} sectors, {} bytes/sector)",
            nsid,
            ctrl.sector_count,
            ctrl.sector_size
        );
    ...
    fn ring_db(&self, qid: u32, sq: bool, val: u32) {
        ...
        // SAFETY: ptr is computed from regs base + doorbell offset.
        // The offset is validated by the NVMe spec (0x1000 + stride).
        unsafe {
            (*ptr).write(val);
    ...
    fn submit(sq_tail: &mut u32, cmd: &NvmeCmd, sq: &RingBuf) {
        ...
        // SAFETY: sq.entry(tail) returns a pointer to a valid slot in the
        // DMA ring buffer. NvmeCmd is 64 bytes, which is the entry_size.
        unsafe {
            core::ptr::copy_nonoverlapping(
                cmd as *const _ as *const u8,
                sq.entry(tail),
                core::mem::size_of::<NvmeCmd>(),
            );
        }
        *sq_tail += 1;
    }
    ...
    fn reap(cq_head: &mut u32, phase: &mut u8, cq: &RingBuf, _qid: u32) -> bool {
        ...
        // SAFETY: cq.entry(head) is a valid slot in the completion ring.
        // read_volatile reads directly from DMA memory (MMIO).
        ...
        *cq_head += 1;
        if (*cq_head).is_multiple_of(cq.num_entries) {
            *phase ^= 1;
        }
        true
    }
    ...
    fn reap_with_timeout(cq_head: &mut u32, phase: &mut u8, cq: &RingBuf, timeout: u32) -> bool {
        for _ in 0..timeout {
            if Self::reap(cq_head, phase, cq, 0) {
                return true;
            }
            core::hint::spin_loop();
        }
    ...
    fn io_cmd(&mut self, opcode: u8, nsid: u32, prp1: u64, lba: u64, count: u32) -> bool {
        let cmd = NvmeCmd {
            cdw0: (opcode as u32) | ((self.next_cid as u32) << 18),
            nsid,
            rsvd2: 0,
    ...
    fn identify_nsid(&mut self) -> u32 {
        ...
        // SAFETY: buf is a 4096-byte DMA buffer. Byte 0x504 is the Number
        // of Namespaces field in NVMe Identify Controller data.
    ...
    fn identify_ns(&mut self, nsid: u32) -> u64 {
        ...
        // SAFETY: buf is a 4096-byte DMA buffer. Bytes 0-7 are the
        // namespace size (in blocks) in NVMe Identify Namespace data.
    ...
    fn identify_sector_size(&mut self, nsid: u32) -> u16 {
        ...
        // SAFETY: buf is a 4096-byte DMA buffer. Byte 26 is FLBAS.
        let flbas = unsafe { *(buf.as_ptr().add(26) as *const u8) };
        let lbaf_idx = (flbas & 0x0F) as usize;
        // SAFETY: LBAF0 starts at byte 128, each entry is 4 bytes.
        // lbaf_idx < 16 (from FLBAS bits 3:0), so offset < 128+64=192 < 4096.
    ...
    pub fn smart_health(&mut self) -> Option<NvmeHealth> {
        ...
        // SAFETY: buf is a 4096-byte DMA buffer containing NVMe SMART/Health
        // log page data. All field offsets are per NVMe spec §5.14. read_volatile
        // reads directly from DMA memory. Offsets verified to be < 4096.
        unsafe {
            let ptr = buf.as_ptr();
    ...
    fn setup_interrupts(&mut self, bus: u8, slot: u8, func: u8) -> u8 {
        ...
        if let Some(msix_cap) = vahi_pci::find_capability(bus, slot, func, PCI_CAP_MSIX) {
            ...
            if let Some(base_vec) = vahi_apic::msi::alloc() {
                // Enable MSI-X
                vahi_pci::write_config_u16(bus, slot, func, msix_cap + 2, msg_ctrl | (1 << 15));
                crate::serial_write("");
                return base_vec;
            }
        }
...
impl BlockDevice for NvmeDisk {
    fn read_sector(&mut self, sector: u64, buf: &mut [u8]) -> Result<(), BlockDeviceError> {
        ...
        let dma = PooledDma::alloc(ssz, self.ctrl.bdf).ok_or(BlockDeviceError::ReadError)?;
        if !self
            .ctrl
            .io_cmd(IO_READ, self.ctrl.nsid, dma.phys(), sector, 1)
        {
            return Err(BlockDeviceError::ReadError);
        }
        let len = core::cmp::min(buf.len(), ssz);
        // SAFETY: DMA buffer contains valid sector data from the NVMe read.
        // len = min(buf.len(), sector_size), both bounded by DMA allocation.
        unsafe {
            core::ptr::copy_nonoverlapping(dma.as_ptr(), buf.as_mut_ptr(), len);
        }
        Ok(())
    }
    ...
    fn write_sector(&mut self, sector: u64, buf: &[u8]) -> Result<(), BlockDeviceError> {
        let ssz = self.ctrl.sector_size() as usize;
        let mut dma = PooledDma::alloc(ssz, self.ctrl.bdf).ok_or(BlockDeviceError::WriteError)?;
        let len = core::cmp::min(buf.len(), ssz);
        // SAFETY: buf has len bytes (min(buf.len(), sector_size)).
        // DMA buffer is sector_size bytes.
        unsafe {
            core::ptr::copy_nonoverlapping(buf.as_ptr(), dma.as_mut_ptr(), len);
        }
        if !self
            .ctrl
            .io_cmd(IO_WRITE, self.ctrl.nsid, dma.phys(), sector, 1)
        {
