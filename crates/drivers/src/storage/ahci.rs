...
use crate::block::{register_block_device, BlockDevice, BlockDeviceError};
use alloc::sync::Arc;
...
impl AhciPort {
    pub fn new(
        ...
        virt_clb: *mut CommandHeader,
        virt_ctba: *mut HbaCmdTable,
    ) -> Self {
...
pub fn init(base_addr: usize) {
    // SAFETY: base_addr is the MMIO base from BAR5, obtained during PCI
    // enumeration. HbaMemory maps the AHCI HBA registers. The pointer
    // is valid for the lifetime of the kernel (MMIO region is never unmapped).
    ...
    for i in 0..32 {
        if pi & (1 << i) != 0 {
            ...
            if let PortType::SATA = port_type {
                ...
                // SAFETY: port_ptr is &mut hba.ports[i] — a valid MMIO-mapped
                // HbaPort within the HBA register block.
...
pub fn configure_port(port: &mut HbaPort) -> (*mut CommandHeader, *mut HbaCmdTable) {
    ...
    let cmd_list_phys = vahi_memory::virt_to_phys(cmd_list_virt)
        .expect("Failed to get physical address for AHCI CLB");
    ...
    let fis_phys =
        vahi_memory::virt_to_phys(fis_virt).expect("Failed to get physical address for AHCI FB");
    ...
    // SAFETY: cmd_list is a Box::leak'd 4096-byte buffer, 1K-aligned.
    // CommandHeader is 32 bytes, 32 entries = 1024 bytes, fits in the buffer.
    let cmd_headers =
        unsafe { core::slice::from_raw_parts_mut(cmd_list.as_mut_ptr() as *mut CommandHeader, 32) };
    ...
    let offset = vahi_memory::physical_memory_offset();
...
impl AhciPort {
    pub fn read(&mut self, start_lba: u64, sectors: u32, buf: &mut [u8]) -> bool {
        // SAFETY: self.port is a valid MMIO-mapped HbaPort pointer, set during
        // AhciPort::new from the HBA port array. Never freed (MMIO region persists).
        ...
        // SAFETY: clb_virt = clb_phys + HHDM offset (identity-mapped).
        // slot < 32, CommandHeader is 32 bytes, total 1024 bytes fits in CLB.
        ...
        cmd_header
            .dw0
            .write(::core::mem::size_of::<FisRegH2D>() as u32 / 4); // 0 = Read (Write bit clear)
        cmd_header.dw1.write(1); // 1 PRDT Entry

        let ctba_virt = self.ctba_virt as u64;

        // Clear the table (memset)
        // SAFETY: ctba_virt was allocated by Box::leak during configure_port.
        // write_bytes zeros the full command table before use.
        unsafe {
            core::ptr::write_bytes(
                ctba_virt as *mut u8,
                0,
                ::core::mem::size_of::<HbaCmdTable>(),
            );
        }

        // SAFETY: ctba_virt points to a valid HbaCmdTable allocation.
        ...
        let buf_phys = vahi_memory::virt_to_phys_dma(buf_virt_addr);
        ...
        let (dma_virt, dma_phys) = if crosses {
            ...
            // SAFETY: layout is 4096-aligned, total_len bytes. alloc_zeroed
            // returns a valid pointer or null (we'd panic on null in practice).
            let bounce = unsafe { alloc::alloc::alloc_zeroed(layout) };
            let bounce_phys = vahi_memory::virt_to_phys_dma(VirtAddr::from_ptr(bounce)).as_u64();
            (bounce, bounce_phys)
        } else {
        ...
        // Setup Command FIS
        // SAFETY: cfis is 64 bytes, FisRegH2D is 20 bytes. Cast is valid
        // as FisRegH2D is repr(C) and smaller than cfis.
        ...
        if crosses {
            // SAFETY: dma_virt was allocated with this layout. total_len bytes
            // copied back from bounce buffer to the caller's buffer.
            unsafe {
                core::ptr::copy_nonoverlapping(dma_virt, buf.as_mut_ptr(), total_len);
            }
            let layout = alloc::alloc::Layout::from_size_align(total_len, 4096).unwrap();
            // SAFETY: dma_virt was allocated with this exact layout above.
            // SAFETY: dma_virt was allocated with this exact layout above.
            unsafe {
                alloc::alloc::dealloc(dma_virt, layout);
            }
        }
        true
    }
    ...
    pub fn identify_device(&mut self) -> bool {
        // SAFETY: self.port is a valid MMIO-mapped HbaPort pointer.
        let port = unsafe { &mut *self.port };
        let offset = vahi_memory::physical_memory_offset();
        ...
        let clb_phys = port.clb.read() as u64 | ((port.clbu.read() as u64) << 32);
        let clb_virt = clb_phys + offset;
        // SAFETY: clb_virt is identity-mapped CLB. slot < 32.
        let cmd_header = unsafe { &mut *(clb_virt as *mut CommandHeader).offset(slot as isize) };
        ...
        let buf_phys = vahi_memory::virt_to_phys_dma(buf_virt);

        cmd_header
            .dw0
            .write(core::mem::size_of::<FisRegH2D>() as u32 / 4); // Read
        cmd_header.dw1.write(1); // 1 PRDT entry

        let ctba_virt = self.ctba_virt as u64;
        // SAFETY: ctba_virt is the Box::leak'd command table from configure_port.
        unsafe {
        ...
        // SAFETY: cfis is 64 bytes, FisRegH2D fits within.
        let fis = unsafe { &mut *(cmd_tbl.cfis.as_mut_ptr() as *mut FisRegH2D) };
        fis.fis_type = 0x27;
        ...
        // SAFETY: buf is 512 bytes (256 u16 words). IDENTIFY data layout
        // has 256 words. buf was allocated and zeroed by Box::leak.
        ...
        if total_sectors > 0 {
            self.identify_sector_count
                .store(total_sectors, core::sync::atomic::Ordering::SeqCst);
            // println removed during extraction
        } else {
            ...
            if lba28 > 0 {
                self.identify_sector_count
                    .store(lba28, core::sync::atomic::Ordering::SeqCst);
                // println removed during extraction
            }
        }

        // Free the temporary buffer
        // SAFETY: buf was created by Box::leak(Box::new([0u8; 512])).
        // from_raw takes ownership and deallocates. The IDENTIFY data has
        // already been parsed into self.identify_sector_count.
        unsafe {
            let _ = alloc::boxed::Box::from_raw(buf);
        }
    ...
    pub fn write(&mut self, start_lba: u64, sectors: u32, buf: &[u8]) -> bool {
        ...
        cmd_header
            .dw0
            .write((::core::mem::size_of::<FisRegH2D>() as u32 / 4) | (1 << 6));
        cmd_header.dw1.write(1); // 1 PRDT Entry

        let ctba_virt = self.ctba_virt as u64;
        // SAFETY: ctba_virt is Box::leak'd command table from configure_port.
        unsafe {
            core::ptr::write_bytes(
                ctba_virt as *mut u8,
                0,
        ...
        let (dma_virt, dma_phys) = if crosses {
            ...
            // returns a valid pointer or null (we'd panic on null in practice).
            let bounce = unsafe { alloc::alloc::alloc_zeroed(layout) };
            // SAFETY: buf has total_len bytes. bounce is zeroed, total_len bytes.
            unsafe {
                core::ptr::copy_nonoverlapping(buf.as_ptr(), bounce, total_len);
            }
            let bounce_phys = vahi_memory::virt_to_phys_dma(VirtAddr::from_ptr(bounce)).as_u64();
            (bounce, bounce_phys)
        } else {
