// SAFETY CONTRACT:
// PATA (IDE) controller uses fixed I/O ports 0x1F0-0x1F7 (primary) and
// 0x376/0x3F6 (control). All access is direct I/O. Commands are issued
// by writing to command register and polling status register. DRQ bit
// (status bit 3) indicates data transfer ready. BSY bit (status bit 7)
// must clear before next command.
...
fn probe_master() -> Option<PataDevice> {
    unsafe {
        ...
        if status.read() & STATUS_ERR != 0 {
            return None;
        }

        let mut timeout = 1_000_000i32;
        while timeout > 0 {
            if status.read() & STATUS_DRQ != 0 {
                break;
            }
            timeout -= 1;
            core::hint::spin_loop();
        ...
        crate::serial_write(&alloc::format!(
            "[PATA] Drive detected: {} sectors ({:.1} MB)\n",
            sectors,
            (sectors * 512) as f64 / 1_048_576.0
        ));
...
pub fn test_read_sector0() -> Result<(), &'static str> {
    let devices = crate::block::BLOCK_DEVICES.lock();
    if devices.is_empty() {
    ...
    dev.lock()
        ...
        .map_err(|_| "read_sector(0) failed")?;
