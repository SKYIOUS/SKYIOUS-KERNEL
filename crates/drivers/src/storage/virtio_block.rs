...
// SAFETY CONTRACT:
// Port I/O accesses VirtIO-blk registers at fixed offsets from base_addr.
// DMA ring buffers (descriptor table, available ring, used ring) are allocated
// via Box::leak with physical addresses provided to the device via queue_pfn.
// All DMA buffers must be physically contiguous (buddy allocator guarantee).

use crate::block::{register_block_device, BlockDevice, BlockDeviceError};
use crate::net::virtio::{
    VirtIOQueue, REG_DEVICE_STATUS, REG_QUEUE_NOTIFY, REG_QUEUE_PFN, REG_QUEUE_SEL, REG_QUEUE_SIZE,
...
impl VirtIOBlock {
    pub fn new(base_addr: u16) -> Self {
        ...
        unsafe {
            queue_pfn_port.write(queue.pfn());
        }
        ...
        let capacity =
            unsafe { (cap_port_low.read() as u64) | ((cap_port_high.read() as u64) << 32) };
...
impl BlockDevice for VirtIOBlock {
    fn read_sector(&mut self, sector: u64, buf: &mut [u8]) -> Result<(), BlockDeviceError> {
        ...
        let header_phys = vahi_memory::virt_to_phys(VirtAddr::from_ptr(header_ptr)).unwrap();

        // 2. Prepare Status byte
        let status = Box::new(0u8);
        let status_ptr = Box::into_raw(status);
        let status_phys = vahi_memory::virt_to_phys(VirtAddr::from_ptr(status_ptr)).unwrap();

        let data_phys = vahi_memory::virt_to_phys(VirtAddr::from_ptr(buf.as_ptr())).unwrap();
        ...
        self.queue.available.ring[self.queue.available.idx as usize % self.queue.size as usize] =
            head as u16;
