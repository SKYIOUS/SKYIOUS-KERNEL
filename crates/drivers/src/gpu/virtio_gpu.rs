// SAFETY CONTRACT:
// VirtIO-GPU uses I/O port base from PCI BAR1 for control queues.
// DMA control queue buffers are allocated via Box::leak with physical
// addresses provided to the device via queue_pfn. Framebuffer is mapped
// via VirtIO-GPU resource attach — the physical address is provided in
// the control request and the device maps it into its display pipeline.
use alloc::boxed::Box;
use alloc::vec;
...
fn phys_of<T>(x: &T) -> u64 {
    vahi_memory::virt_to_phys(VirtAddr::from_ptr(x as *const T))
        .expect("virtio_gpu: cannot translate")
        .as_u64()
}

fn phys_of_mut<T>(x: &mut T) -> u64 {
    vahi_memory::virt_to_phys(VirtAddr::from_ptr(x as *mut T))
        .expect("virtio_gpu: cannot translate")
        .as_u64()
}
...
impl VirtioGpu {
    pub fn new(io_base: u16) -> Self {
        ...
        unsafe {
            qpfn.write((phys_of(&descs[0]) >> 12) as u32);
        }

        unsafe {
            status.write(STATUS_ACK | STATUS_DRIVER | STATUS_FEATURES_OK | STATUS_DRIVER_OK);
        }
    ...
    fn create_fb(&mut self) {
        ...
        let n_pages = fb_bytes.div_ceil(4096);
...
pub static GPU: Mutex<Option<VirtioGpu>> = Mutex::new(None);

pub fn init(io_base: u16) {
    let gpu = VirtioGpu::new(io_base);
    if let Some(ref fb) = gpu.fb_mem {
        let ptr = fb.as_ptr() as *mut u32;
        crate::graphics::FRAMEBUFFER.store(ptr, core::sync::atomic::Ordering::SeqCst);
        crate::graphics::WIDTH.store(gpu.width as usize, core::sync::atomic::Ordering::SeqCst);
        crate::graphics::HEIGHT.store(gpu.height as usize, core::sync::atomic::Ordering::SeqCst);
        crate::graphics::STRIDE.store(gpu.width as usize, core::sync::atomic::Ordering::SeqCst);
    }
    *GPU.lock() = Some(gpu);
}
