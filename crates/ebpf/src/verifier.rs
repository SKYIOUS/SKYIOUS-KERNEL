...
pub fn verify(insns: &[EbpfInsn]) -> bool {
    ...
    for (i, insn) in insns.iter().enumerate() {
        ...
        if (cls == BPF_ALU || cls == BPF_ALU64 || cls == BPF_JMP || cls == BPF_JMP32) && dst == 10 {
            return false;
        }
        match cls {
            BPF_LD => {
                if insn.code & 0xe0 != 0x00 {
                    return false;
                }
            ...
            BPF_LDX => {
                if (insn.off as i64) < -512 || (insn.off as i64) > 512 {
                    return false;
                }
                if dst == 10 {
                    return false;
...
impl VState {
    fn new() -> Self {
        VState {
            regs: [RegState { tnum: Tnum::any() }; 11],
            stack: [Tnum::any(); 64],
        }
    }
    fn init() -> Self {
        let mut s = Self::new();
        s.regs[10] = RegState::exact(STACK_SIZE as u64);
        s
    }
    fn write_stack(&mut self, off: u64, size: u64, t: Tnum) {
        if !off.is_multiple_of(8) || !size.is_multiple_of(8) {
            return;
        }
    ...
    fn merge(&self, o: &Self) -> Self {
        ...
        for i in 0..11 {
            m.regs[i].tnum = self.regs[i].tnum.merge(o.regs[i].tnum);
        }
        for i in 0..64 {
            m.stack[i] = self.stack[i].merge(o.stack[i]);
        }
        m
    }
    fn eq(&self, o: &Self) -> bool {
        for i in 0..11 {
            if self.regs[i].tnum != o.regs[i].tnum {
                return false;
            }
        }
        for i in 0..64 {
            if self.stack[i] != o.stack[i] {
                return false;
            }
        }
        true
...
fn alu_op_tnum(op: u8, dst: Tnum, src: Tnum, is_alu32: bool) -> Tnum {
    let r = match op {
        ...
        BPF_AND => dst.and(src),
        BPF_LSH => dst.shl(src),
        BPF_RSH => dst.lshr(src),
        BPF_NEG => dst.neg(),
        BPF_MOD => Tnum::any(),
        BPF_XOR => dst.xor(src),
        BPF_MOV => src,
        BPF_ARSH => dst.ashr(src),
        _ => Tnum::any(),
    };
    if is_alu32 {
        let lo = 0xFFFF_FFFFu64;
        Tnum {
            value: r.value & lo & !r.mask,
            mask: r.mask & lo | !lo,
        }
    } else {
        r
...
pub fn tnum_verify(insns: &[EbpfInsn]) -> bool {
    ...
    worklist.push(0);
    while let Some(cur) = worklist.pop() {
        let Some(pre) = state_map.get(&cur).cloned() else {
            continue;
        };
        let insn = &insns[cur];
        let cls = insn.code & 0x07;
        ...
        match cls {
            BPF_ALU | BPF_ALU64 => {
                ...
                let src_tnum = if use_imm {
                    if is64 {
                        Tnum::exact(insn.imm as i64 as u64)
                    } else {
                        Tnum::exact(insn.imm as u32 as u64)
                    }
                } else {
                    if is64 {
                        pre.regs[insn.src_reg as usize].tnum
                    } else {
                        mask32(pre.regs[insn.src_reg as usize].tnum)
                    }
                };
                let dst_tnum = if is64 {
                    pre.regs[dst].tnum
                } else {
                    mask32(pre.regs[dst].tnum)
                };
                if (op == BPF_DIV || op == BPF_MOD) && src_tnum.could_be_zero() {
                    return false;
                }
                post.regs[dst].tnum = alu_op_tnum(op, dst_tnum, src_tnum, !is64);
                let next = cur + 1;
                if next < insns.len() {
                    push_merged(&mut state_map, &mut worklist, next, &post);
                }
            }
            BPF_LDX => {
                let base = &pre.regs[insn.src_reg as usize].tnum;
                let size = sz_bytes(insn.code);
                if !check_stack_acc(base, insn.off, size) {
                    return false;
                }
                let mut post = pre.clone();
            ...
            BPF_ST => {
                ...
                let size = if insn.code & BPF_SIZE_MASK == 0x00 {
                    8
                } else {
                    4
                };
            ...
            BPF_LD if insn.code & BPF_SIZE_MASK == BPF_DW && insn.code & 0xe0 == 0x00 => {
                if cur + 1 >= insns.len() {
                    return false;
                }
                let n = &insns[cur + 1];
    ...
    match op {
        BPF_JEQ | BPF_JNE => {
            ...
            if use_imm && eq {
                ...
                let r = d.intersect(s);
                state.regs[dst].tnum = r;
                state.regs[insn.src_reg as usize].tnum = r;
            }
        }
        _ => {}
    }
...
fn push_merged(
    state_map: &mut BTreeMap<usize, VState>,
    ...
    match state_map.get(&target).cloned() {
        Some(existing) => {
            ...
            if !existing.eq(&merged) {
                state_map.insert(target, merged);
                worklist.push(target);
            }
        }
        None => {
            state_map.insert(target, new_state.clone());
            worklist.push(target);
        }
    }
}
