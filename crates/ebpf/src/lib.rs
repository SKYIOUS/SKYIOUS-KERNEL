//! # vahi-ebpf — eBPF Virtual Machine & JIT Compiler
//!
//! User-space eBPF bytecode execution for seccomp filters, network
//! packet classification, and tracing probes.
//!
//! ## Exports
//!
//! | Module | Purpose |
//! |--------|---------|
//! | `vm` | eBPF register machine: execute verified bytecode with 11 registers |
//! | `verifier` | Static verifier: ensures programs terminate and access memory safely |
//! | `maps` | eBPF map types: hash, array, per-CPU array |
//! | `helpers` | eBPF helper functions: get_prandom, get_smp_processor_id, etc. |
//! | `jit` | x86_64 JIT compiler: compiles eBPF bytecode to native machine code |
//! | `tnum` | Tracked numbers: abstract domain for verifier range tracking |
//!
//! ## Usage
//!
//! ```text
//! // Typical flow:
//! // 1. Verifier validates the program (no OOB, loops bounded, helpers valid)
//! // 2. JIT compiles to x86_64 native code (or falls back to interpreter)
//! // 3. VM executes with per-packet context (skb data, hooks, maps)
//! ```
//!
//! ## Invariants
//!
//! - Verifier rejects programs with unbounded loops or OOB memory access
//! - JIT output must pass x86_64 calling convention (preserves rbx, r12-r15)
//! - All map operations are bounds-checked before access
//! - Helper function calls are whitelisted (no arbitrary function pointers)
...
use verifier::{tnum_verify, verify};
use vm::{EbpfInsn, EbpfRegs, EbpfVm, STACK_SIZE};
...
pub fn sys_bpf(cmd: u32, arg1: u64, arg2: u64, arg3: u64) -> u64 {
    ...
    if vahi_syscalls::get_current_euid() != 0
        && !vahi_syscalls::has_capability(vahi_syscalls::CAP_SYS_ADMIN)
    {
...
fn bpf_map_create(attr_ptr: *const u8) -> u64 {
    ...
    if unsafe { vahi_syscalls::user_access::copy_from_user(&mut attr_buf, attr_ptr).is_err() } {
        return Errno::EFAULT as u64;
    }
    let attr: &BpfMapCreateAttr = unsafe { &*(attr_buf.as_ptr() as *const BpfMapCreateAttr) };
    if attr.map_type >= maps::MAX_MAP_TYPE_COUNT
        || attr.key_size == 0
        || attr.value_size == 0
        || attr.max_entries == 0
    {
    ...
    let map: Arc<dyn Map> = match attr.map_type {
        t if t == BPF_MAP_TYPE_HASH => Arc::new(maps::HashTable::new(
            attr.key_size,
            attr.value_size,
            attr.max_entries,
        )),
        t if t == BPF_MAP_TYPE_ARRAY => {
            Arc::new(maps::ArrayMap::new(attr.value_size, attr.max_entries))
        }
        t if t == BPF_MAP_TYPE_PERF_EVENT_ARRAY => {
            Arc::new(maps::PerfEventArray::new(attr.max_entries))
        }
        t if t == BPF_MAP_TYPE_RINGBUF => Arc::new(maps::RingBuf::new(
            attr.max_entries as usize * attr.value_size as usize,
        )),
...
fn bpf_map_lookup_elem(map_id: u64, key_ptr: *const u8, value_ptr: *mut u8) -> u64 {
    ...
    match map {
        Some(m) => {
            ...
            if unsafe { vahi_syscalls::user_access::copy_from_user(&mut key, key_ptr).is_err() } {
                return Errno::EFAULT as u64;
            }
            match m.lookup(&key) {
                Some(val) => {
                    ...
                    if unsafe {
                        vahi_syscalls::user_access::copy_to_user(value_ptr, &val[..copy_len])
                    }
                    .is_err()
                    {
                        return Errno::EFAULT as u64;
...
fn bpf_map_update_elem(map_id: u64, key_ptr: *const u8, value_ptr: *const u8) -> u64 {
    ...
    match map {
        Some(m) => {
            ...
            if unsafe { vahi_syscalls::user_access::copy_from_user(&mut value, value_ptr).is_err() }
            {
                return Errno::EFAULT as u64;
            }
...
fn bpf_prog_load(attr_ptr: *const u8, _prog_type: u64) -> u64 {
    ...
    if unsafe { vahi_syscalls::user_access::copy_from_user(&mut insns_buf, attr.insns).is_err() } {
        return Errno::EFAULT as u64;
    }
    let insns: &[vm::EbpfInsn] = unsafe {
        slice::from_raw_parts(
            insns_buf.as_ptr() as *const vm::EbpfInsn,
            attr.insn_cnt as usize,
        )
    };

    let license = unsafe {
        vahi_syscalls::user_access::read_user_string(attr.license, 128).unwrap_or_default()
    };
...
fn bpf_prog_attach(prog_id: u64, attach_type: u32, target: u64) -> u64 {
    ...
    let target_name = if target != 0 {
        unsafe {
            vahi_syscalls::user_access::read_user_string(target as *const u8, 256)
                .unwrap_or_default()
        }
    } else {
