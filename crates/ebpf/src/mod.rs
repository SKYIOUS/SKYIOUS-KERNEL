        ...
        Some(m) => {
            ...
            match m.lookup(&key) {
                Some(val) => {
                    ...
                    if unsafe { vahi_syscalls::user_access::copy_to_user(value_ptr, &val[..copy_len]) }.is_err() {
                        return Errno::EFAULT as u64;
                    }
                    0
                }
...
fn bpf_prog_load(attr_ptr: *const u8, _prog_type: u64) -> u64 {
    ...
    if unsafe { vahi_syscalls::user_access::copy_from_user(&mut insns_buf, attr.insns as *const u8).is_err() } {
        return Errno::EFAULT as u64;
    }
    ...
    let license = unsafe { vahi_syscalls::user_access::read_user_string(attr.license, 128).unwrap_or_default() };
...
fn bpf_prog_attach(prog_id: u64, attach_type: u32, target: u64) -> u64 {
    ...
    let target_name = if target != 0 {
        unsafe { vahi_syscalls::user_access::read_user_string(target as *const u8, 256).unwrap_or_default() }
    } else {
