...
pub fn bpf_helper_map_lookup_elem(
    ...
    match maps {
        Some(m) => {
            ...
            if key_off.checked_add(ksz).is_none_or(|e| e > STACK_SIZE) {
                return -1;
            }
            if val_off.checked_add(vsz).is_none_or(|e| e > STACK_SIZE) {
                return -1;
            }
...
pub fn bpf_helper_debug_print(stack: &[u8; STACK_SIZE], msg_off: usize, len: usize) {
    if msg_off.checked_add(len).is_none_or(|e| e > STACK_SIZE) {
        return;
    }
    let _s = match core::str::from_utf8(&stack[msg_off..msg_off + len]) {
        Ok(s) => s,
        Err(_) => return,
    };
}
...
pub static TAIL_CALL_PROGS: vahi_sync::IrqSafeMutex<alloc::collections::VecDeque<u32>> =
    vahi_sync::IrqSafeMutex::new(alloc::collections::VecDeque::new());
